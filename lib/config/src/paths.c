...
char *
clawd_paths_config_dir(void)
{
    const char *env;

    env = getenv("CLAWD_CONFIG_DIR");
    if (env && env[0])
        return strdup(env);

    env = getenv("XDG_CONFIG_HOME");
    if (env && env[0])
        return join_path(env, "clawd");

    char *home = get_home();
    if (!home)
        return NULL;
    char *cfg = join_path(home, ".config/clawd");
    free(home);
    return cfg;
}

char *
clawd_paths_data_dir(void)
{
    const char *env;

    env = getenv("CLAWD_DATA_DIR");
    if (env && env[0])
        return strdup(env);
    ...
    if (!home)
        return NULL;
    char *data = join_path(home, ".local/share/clawd");
    free(home);
    return data;
}

char *
clawd_paths_runtime_dir(void)
{
    const char *env;

    env = getenv("CLAWD_RUNTIME_DIR");
    if (env && env[0])
        return strdup(env);
    ...
    return strdup("/run/clawd");
}

char *
clawd_paths_socket(void)
{
    char *rtdir = clawd_paths_runtime_dir();
    if (!rtdir)
        return NULL;
    char *sock = join_path(rtdir, "clawd.sock");
    free(rtdir);
    return sock;
}

int
clawd_paths_ensure_dirs(void)
{
    int rc = 0;

    char *cfg = clawd_paths_config_dir();
    char *data = clawd_paths_data_dir();
    char *run  = clawd_paths_runtime_dir();
...
char *
clawd_paths_expand(const char *path)
{
    if (!path)
        return NULL;
