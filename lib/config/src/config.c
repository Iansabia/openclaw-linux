...
env_subst(const char *input)
{
    ...
    return clawd_paths_expand(input);
}
...
static void
apply_defaults(clawd_config_t *cfg)
{
    if (!cfg->config_dir)
        cfg->config_dir = clawd_paths_config_dir();
    if (!cfg->data_dir)
        cfg->data_dir = clawd_paths_data_dir();
    if (!cfg->runtime_dir)
        cfg->runtime_dir = clawd_paths_runtime_dir();
    if (!cfg->profile)
    ...
    if (!cfg->gateway.socket_path)
        cfg->gateway.socket_path = clawd_paths_socket();
    ...
    if (cfg->logging.level == 0)
        cfg->logging.level = CLAWD_LOG_INFO;
}
...
/**
 * Populate a clawd_config_t from a cJSON object tree.
 * Only sets fields that are present in the JSON; does NOT reset unset fields.
 */
static void
populate_from_json(clawd_config_t *cfg, const cJSON *root)
{
    ...
    if (log_sec && cJSON_IsObject(log_sec)) {
        ...
        if (lvl) {
            if (cJSON_IsNumber(lvl)) {
                ...
                if      (strcasecmp(s, "emerg")   == 0) cfg->logging.level = CLAWD_LOG_EMERG;
                else if (strcasecmp(s, "alert")   == 0) cfg->logging.level = CLAWD_LOG_ALERT;
                else if (strcasecmp(s, "crit")    == 0) cfg->logging.level = CLAWD_LOG_CRIT;
                else if (strcasecmp(s, "err")     == 0 ||
                         strcasecmp(s, "error")   == 0) cfg->logging.level = CLAWD_LOG_ERR;
                else if (strcasecmp(s, "warning") == 0 ||
                         strcasecmp(s, "warn")    == 0) cfg->logging.level = CLAWD_LOG_WARNING;
                else if (strcasecmp(s, "notice")  == 0) cfg->logging.level = CLAWD_LOG_NOTICE;
                else if (strcasecmp(s, "info")    == 0) cfg->logging.level = CLAWD_LOG_INFO;
                else if (strcasecmp(s, "debug")   == 0) cfg->logging.level = CLAWD_LOG_DEBUG;
            }
        }
 ...
 *
 * E.g. if base is /etc/clawd/clawd.yaml and profile is "dev",
 * look for /etc/clawd/profiles/dev.yaml.
 */
static void
load_profile_overlay(clawd_config_t *cfg, const char *base_path)
{
...
int
clawd_config_load(const char *path, clawd_config_t *cfg)
{
    if (!path || !cfg)
        return -1;
...
int
clawd_config_load_default(clawd_config_t *cfg)
{
    if (!cfg)
        return -1;
    ...
    const char *env_dir = getenv("CLAWD_CONFIG_DIR");
    if (env_dir && env_dir[0]) {
        snprintf(path_env, sizeof(path_env), "%s/clawd.yaml", env_dir);
        candidates[0] = path_env;
    }

    /* 2. ~/.config/clawd/clawd.yaml  (or $XDG_CONFIG_HOME/clawd/clawd.yaml) */
    {
        char *cdir = clawd_paths_config_dir();
        if (cdir) {
            snprintf(path_home, sizeof(path_home), "%s/clawd.yaml", cdir);
    ...
    candidates[2] = "/etc/clawd/clawd.yaml";
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (access(candidates[i], R_OK) == 0) {
            int rc = clawd_config_load(candidates[i], cfg);
...
void
clawd_config_free(clawd_config_t *cfg)
{
    if (!cfg)
        return;
...
const char *
clawd_config_get_string(const clawd_config_t *cfg, const char *key)
{
    if (!cfg || !key)
        return NULL;
...
int
clawd_config_get_int(const clawd_config_t *cfg, const char *key, int def)
{
    if (!cfg || !key)
        return def;
...
bool
clawd_config_get_bool(const clawd_config_t *cfg, const char *key, bool def)
{
    if (!cfg || !key)
        return def;
...
parse_log_level(const char *s)
{
    ...
    if (strcasecmp(s, "emerg")   == 0) return CLAWD_LOG_EMERG;
    if (strcasecmp(s, "alert")   == 0) return CLAWD_LOG_ALERT;
    if (strcasecmp(s, "crit")    == 0) return CLAWD_LOG_CRIT;
    if (strcasecmp(s, "err")     == 0 ||
        strcasecmp(s, "error")   == 0) return CLAWD_LOG_ERR;
    if (strcasecmp(s, "warning") == 0 ||
        strcasecmp(s, "warn")    == 0) return CLAWD_LOG_WARNING;
    if (strcasecmp(s, "notice")  == 0) return CLAWD_LOG_NOTICE;
    if (strcasecmp(s, "info")    == 0) return CLAWD_LOG_INFO;
    if (strcasecmp(s, "debug")   == 0) return CLAWD_LOG_DEBUG;
...
int
clawd_config_merge_env(clawd_config_t *cfg)
{
    if (!cfg)
        return 0;
    ...
    if ((v = getenv("CLAWD_HOST")))         set_str(&cfg->gateway.host, v);
    if ((v = getenv("CLAWD_SOCKET")))       set_str(&cfg->gateway.socket_path, v);
    if ((v = getenv("CLAWD_TLS_CERT")))     set_str(&cfg->gateway.tls_cert, v);
    if ((v = getenv("CLAWD_TLS_KEY")))      set_str(&cfg->gateway.tls_key, v);
    ...
    if ((v = getenv("CLAWD_PORT"))) {
        int p = atoi(v);
        if (p > 0 && p <= 65535)
            cfg->gateway.port = p;
    }

    if ((v = getenv("CLAWD_MAX_TOKENS"))) {
        int mt = atoi(v);
        if (mt > 0)
            cfg->model.max_tokens = mt;
    }

    if ((v = getenv("CLAWD_TEMPERATURE"))) {
        float t = (float)atof(v);
        if (t >= 0.0f)
            cfg->model.temperature = t;
    }

    if ((v = getenv("CLAWD_SANDBOX")))
        cfg->security.sandbox_enabled = parse_bool_env(v);

    if ((v = getenv("CLAWD_LOG_LEVEL"))) {
    ...
    if (getenv("CLAWD_TLS_CERT") && getenv("CLAWD_TLS_KEY"))
...
int
clawd_config_validate(const clawd_config_t *cfg)
{
    if (!cfg)
        return -1;
