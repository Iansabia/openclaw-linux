 ...
 */
static int
check_type(const cJSON *item, clawd_schema_type_t type)
{
    switch (type) {
    case SCHEMA_STRING:
 ...
 */
static const char *
type_name(clawd_schema_type_t type)
{
    switch (type) {
...
int
clawd_schema_validate(const clawd_schema_t *schema,
                       const cJSON          *data,
                       char                 *err_buf,
                       size_t                err_len)
{
    ...
    for (int i = 0; i < schema->field_count; i++) {
        const clawd_schema_field_t *f = &schema->fields[i];
...
static const clawd_schema_field_t config_fields[] = {
...
static const clawd_schema_t config_schema = {
    .fields      = config_fields,
    .field_count = (int)(sizeof(config_fields) / sizeof(config_fields[0])),
};

const clawd_schema_t *
clawd_schema_config(void)
{
