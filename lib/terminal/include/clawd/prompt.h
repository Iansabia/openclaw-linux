...
#ifndef CLAWD_PROMPT_H
#define CLAWD_PROMPT_H
 ...
 */
bool clawd_prompt_confirm(const char *message, bool default_yes);
 ...
 */
int clawd_prompt_select(const char *message, const char **options, int count);
 ...
 */
char *clawd_prompt_input(const char *message, const char *default_value);
 ...
 * @return Allocated string (caller must free), or NULL on error/EOF.
 */
char *clawd_prompt_password(const char *message);

#ifdef __cplusplus
}
#endif

#endif /* CLAWD_PROMPT_H */
