 ...
 */
int clawd_seccomp_apply_filter(void)
{
    scmp_filter_ctx ctx = seccomp_init(SCMP_ACT_ERRNO(EPERM));
    if (!ctx) {
        CLAWD_ERROR("seccomp: failed to initialize filter");
        return -1;
    }
    ...
    if (rc < 0) {
        CLAWD_ERROR("seccomp: failed to load filter: %s", strerror(-rc));
        seccomp_release(ctx);
        return -1;
    }

    seccomp_release(ctx);
    CLAWD_DEBUG("seccomp: BPF filter applied successfully");
    return 0;
}
...
int clawd_seccomp_apply_filter(void)
{
    CLAWD_WARN("seccomp: not available on this platform");
    return -1;
}
