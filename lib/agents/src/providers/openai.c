...
static int openai_complete(clawd_provider_t *p,
                           const clawd_completion_opts_t *opts,
                           clawd_completion_t *result);

/* ---- Initialization ----------------------------------------------------- */

int clawd_provider_openai_init(clawd_provider_t *p)
{
    if (!p) return -1;
...
static int parse_response(const char *body, clawd_completion_t *result)
{
    ...
    if (!root) {
        CLAWD_ERROR("openai: failed to parse response JSON");
        return -1;
    }
    ...
    if (error_obj) {
        ...
        CLAWD_ERROR("openai API error: %s", emsg ? emsg : "unknown");
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (usage) {
        result->input_tokens  = clawd_json_get_int(usage, "prompt_tokens", 0);
        result->output_tokens = clawd_json_get_int(usage, "completion_tokens", 0);
    }
    ...
    if (choices && cJSON_IsArray(choices)) {
        ...
        if (choice) {
            ...
            const char *finish = clawd_json_get_string(choice, "finish_reason");
            if (finish) {
            ...
            if (message) {
                ...
                if (tool_calls && cJSON_IsArray(tool_calls)) {
                    ...
                    cJSON_ArrayForEach(tc, tool_calls) {
                        const char *tc_id = clawd_json_get_string(tc, "id");
                        cJSON *func = cJSON_GetObjectItem(tc, "function");
                        if (!func) continue;

                        const char *fname = clawd_json_get_string(func, "name");
                        const char *fargs = clawd_json_get_string(func, "arguments");
                        ...
                        if (msg) {
                            ...
                            clawd_message_append(&result->tool_calls, msg);
                        }
                    }
                }
            }
                           ...
                           clawd_completion_t *result)
{
    ...
    if (!body) {
        CLAWD_ERROR("openai: failed to build request JSON");
        return -1;
    }

    /* Build Authorization header */
    clawd_str_t auth_val = clawd_str_new();
    clawd_str_printf(&auth_val, "Bearer %s", p->api_key ? p->api_key : "");

    clawd_http_header_t *headers = NULL;
    clawd_http_header_add(&headers, "Content-Type", "application/json");
    clawd_http_header_add(&headers, "Authorization", auth_val.data);
    if (p->org_id) {
        clawd_http_header_add(&headers, "OpenAI-Organization", p->org_id);
    }

    clawd_str_free(&auth_val);
    ...
    if (rc == 0 && resp.body) {
        ...
        if (resp_body) {
            ...
            if (resp.status_code >= 200 && resp.status_code < 300) {
                ...
                CLAWD_ERROR("openai: HTTP %d: %s", resp.status_code, resp_body);
                rc = -1;
            }
            free(resp_body);
        } else {
        ...
        CLAWD_ERROR("openai: HTTP request failed");
    }
