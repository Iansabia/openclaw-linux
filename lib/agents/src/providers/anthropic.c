...
static int anthropic_complete(clawd_provider_t *p,
                              const clawd_completion_opts_t *opts,
                              clawd_completion_t *result);

/* ---- Initialization ----------------------------------------------------- */

int clawd_provider_anthropic_init(clawd_provider_t *p)
{
    if (!p) return -1;
...
static const char *role_to_string(clawd_role_t role)
{
    switch (role) {
    case CLAWD_ROLE_USER:      return "user";
    case CLAWD_ROLE_ASSISTANT: return "assistant";
    case CLAWD_ROLE_TOOL:      return "user";  /* tool results sent as user in Anthropic */
    case CLAWD_ROLE_SYSTEM:    return "user";   /* system handled separately */
    }
    return "user";
}
                                 ...
                                 const clawd_completion_opts_t *opts)
{
    ...
    for (clawd_message_t *msg = opts->messages; msg; msg = msg->next) {
        if (msg->role == CLAWD_ROLE_SYSTEM) continue;  /* handled via system field */

        cJSON *jmsg = cJSON_CreateObject();
        if (!jmsg) continue;

        if (msg->role == CLAWD_ROLE_TOOL && msg->tool_call_id) {
        ...
        } else if (msg->role == CLAWD_ROLE_ASSISTANT && msg->tool_name) {
...
static int parse_response(const char *body, clawd_completion_t *result)
{
    ...
    if (!root) {
        CLAWD_ERROR("anthropic: failed to parse response JSON");
        return -1;
    }
    ...
    if (error_obj) {
        ...
        CLAWD_ERROR("anthropic API error: %s", emsg ? emsg : "unknown");
        cJSON_Delete(root);
        return -1;
    }

    /* ID and model */
    const char *id = clawd_json_get_string(root, "id");
    ...
    if (content && cJSON_IsArray(content)) {
        ...
        clawd_message_t *tool_calls = NULL;

        cJSON *block;
        cJSON_ArrayForEach(block, content) {
            const char *type = clawd_json_get_string(block, "type");
            if (!type) continue;
            ...
            if (strcmp(type, "text") == 0) {
                ...
                const char *tool_id   = clawd_json_get_string(block, "id");
                const char *tool_name = clawd_json_get_string(block, "name");
                ...
                if (tc) {
                    ...
                    clawd_message_append(&tool_calls, tc);
                }
            }
        }
...
typedef struct {
    clawd_stream_cb  cb;
    void            *userdata;
    clawd_str_t      event_buf;
    clawd_completion_t *result;
    clawd_str_t      text_accum;
    clawd_message_t *tool_calls;
    /* Current tool use tracking */
    char            *current_tool_id;
    char            *current_tool_name;
    clawd_str_t      current_tool_args;
} sse_ctx_t;

static int handle_sse_event(const clawd_sse_event_t *event, void *userdata)
{
    ...
    const char *type = clawd_json_get_string(json, "type");
    if (!type) {
        cJSON_Delete(json);
        return 0;
    }
    ...
    if (strcmp(type, "content_block_start") == 0) {
        ...
        if (content_block) {
            const char *btype = clawd_json_get_string(content_block, "type");
            if (btype && strcmp(btype, "tool_use") == 0) {
                free(ctx->current_tool_id);
                free(ctx->current_tool_name);
                const char *tid = clawd_json_get_string(content_block, "id");
                const char *tname = clawd_json_get_string(content_block, "name");
                ctx->current_tool_id = tid ? strdup(tid) : NULL;
                ctx->current_tool_name = tname ? strdup(tname) : NULL;
                clawd_str_free(&ctx->current_tool_args);
                ctx->current_tool_args = clawd_str_new();
            }
        }
    } else if (strcmp(type, "content_block_delta") == 0) {
        cJSON *delta = cJSON_GetObjectItem(json, "delta");
        if (delta) {
            const char *dtype = clawd_json_get_string(delta, "type");
            if (dtype && strcmp(dtype, "text_delta") == 0) {
                const char *text = clawd_json_get_string(delta, "text");
                if (text) {
                    clawd_str_append_cstr(&ctx->text_accum, text);
                    if (ctx->cb) {
                        clawd_stream_event_t se = {0};
                        se.type = "text";
                        se.text = text;
                ...
                const char *partial = clawd_json_get_string(delta, "partial_json");
                if (partial) {
                    clawd_str_append_cstr(&ctx->current_tool_args, partial);
                    if (ctx->cb) {
                        clawd_stream_event_t se = {0};
                        se.type = "tool_use";
        ...
        if (ctx->current_tool_name) {
            ...
            if (tc) {
                ...
                clawd_message_append(&ctx->tool_calls, tc);
            } else {
                free(ctx->current_tool_id);
                free(ctx->current_tool_name);
            }
        ...
        cJSON *delta = cJSON_GetObjectItem(json, "delta");
        if (delta) {
            const char *stop = clawd_json_get_string(delta, "stop_reason");
            if (stop && ctx->result) {
        ...
        if (usage && ctx->result) {
            ctx->result->output_tokens = clawd_json_get_int(usage, "output_tokens", 0);
        }
    } else if (strcmp(type, "message_start") == 0) {
        cJSON *message = cJSON_GetObjectItem(json, "message");
        if (message && ctx->result) {
            const char *id = clawd_json_get_string(message, "id");
            const char *model = clawd_json_get_string(message, "model");
            if (id)    ctx->result->id    = strdup(id);
            if (model) ctx->result->model = strdup(model);
            cJSON *usage = cJSON_GetObjectItem(message, "usage");
            if (usage) {
                ctx->result->input_tokens = clawd_json_get_int(usage, "input_tokens", 0);
            }
        }
    } else if (strcmp(type, "error") == 0) {
        cJSON *error_obj = cJSON_GetObjectItem(json, "error");
        const char *emsg = error_obj ?
            clawd_json_get_string(error_obj, "message") : "unknown error";
        CLAWD_ERROR("anthropic stream error: %s", emsg ? emsg : "unknown");
        if (ctx->cb) {
            clawd_stream_event_t se = {0};
            se.type = "error";
            se.text = emsg;
                              ...
                              clawd_completion_t *result)
{
    ...
    if (!body) {
        CLAWD_ERROR("anthropic: failed to build request JSON");
        return -1;
    }
    ...
    if (opts->stream && opts->stream_cb) {
        ...
        ctx.result    = result;
        ctx.text_accum = clawd_str_new();
        ctx.current_tool_args = clawd_str_new();

        rc = clawd_http_sse(&req, handle_sse_event, &ctx);
        ...
        if (rc == 0 && resp.body) {
            ...
            if (resp_body) {
                ...
                if (resp.status_code >= 200 && resp.status_code < 300) {
                    ...
                    CLAWD_ERROR("anthropic: HTTP %d: %s", resp.status_code, resp_body);
                    rc = -1;
                }
                free(resp_body);
            } else {
            ...
            CLAWD_ERROR("anthropic: HTTP request failed");
        }
