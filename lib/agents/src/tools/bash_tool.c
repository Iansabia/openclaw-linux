...
static int bash_exec(clawd_tool_ctx_t *ctx, const char *args_json,
                     clawd_tool_result_t *result)
{
    ...
    const char *command = clawd_json_get_string(args, "command");
    if (!command) {
    ...
    CLAWD_DEBUG("bash tool: executing: %s", command);
    ...
    clawd_proc_result_t proc_result = {0};
    int rc = clawd_proc_exec(&opts, &proc_result);
    ...
    if (rc != 0) {
        ...
        if (proc_result.stdout_data && proc_result.stdout_len > 0) {
            clawd_str_append(&out, proc_result.stdout_data, proc_result.stdout_len);
        }
    ...
    clawd_proc_result_free(&proc_result);
    cJSON_Delete(args);
    return 0;
}
...
const clawd_tool_def_t clawd_bash_tool_def = {
    .name                 = BASH_TOOL_NAME,
