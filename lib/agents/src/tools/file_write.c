...
static int file_write_exec(clawd_tool_ctx_t *ctx, const char *args_json,
                           clawd_tool_result_t *result)
{
    ...
    const char *content = clawd_json_get_string(args, "content");
    if (!content) {
    ...
    CLAWD_DEBUG("file_write: %s (%zu bytes)", file_path, strlen(content));
    ...
    if (!fp) {
        ...
        clawd_str_printf(&err, "error: cannot open file '%s' for writing: %s",
    ...
    if (written != content_len) {
        clawd_str_t err = clawd_str_new();
        clawd_str_printf(&err, "error: wrote %zu of %zu bytes to '%s'",
        ...
        clawd_str_t msg = clawd_str_new();
        clawd_str_printf(&msg, "wrote %zu bytes to %s", written, file_path);
...
const clawd_tool_def_t clawd_file_write_tool_def = {
