...
static void test_exec_echo(void)
{
    ...
    clawd_proc_result_t result = {0};
    int rc = clawd_proc_exec(&opts, &result);
...
static void test_exec_not_found(void)
{
    ...
    char *argv[] = {"__clawd_no_such_command__", NULL};
    clawd_proc_opts_t opts = {0};
    opts.cmd      = "__clawd_no_such_command__";
    opts.argv     = argv;
    opts.set_pgid = true;
...
static void test_spawn_and_wait(void)
{
    ...
    clawd_proc_result_t result = {0};
    int rc = clawd_proc_wait(pid, 5000, &result);
    assert(rc == 0);
...
static void test_kill_tree(void)
{
    ...
    int rc = clawd_proc_kill_tree(pid, SIGKILL);
    assert(rc == 0);
    ...
    assert(!clawd_proc_is_running(pid));
...
static void test_is_running(void)
{
    ...
    assert(clawd_proc_is_running(getpid()));

    /* A very high PID should not exist */
    assert(!clawd_proc_is_running(99999999));
...
static void test_pty_fork_echo(void)
{
    ...
    pid_t pid = clawd_pty_fork(&pty, "echo", argv);
    assert(pid > 0);
    assert(pty.master_fd >= 0);
    ...
    ssize_t n = clawd_pty_read(&pty, buf, sizeof(buf) - 1);
...
static void test_pty_resize(void)
{
    ...
    rc = clawd_pty_resize(&pty, 40, 120);
    assert(rc == 0);
...
static void test_signal_ctx(void)
{
    ...
    clawd_signal_ctx_t *ctx = clawd_signal_ctx_new();
    assert(ctx != NULL);

    int fd = clawd_signal_fd(ctx);
    assert(fd >= 0);

    clawd_signal_ctx_free(ctx);
...
static void test_signal_watch(void)
{
    ...
    int rc = clawd_signal_watch(ctx, SIGUSR1, test_signal_handler, &counter);
    assert(rc == 0);
    ...
    int dispatched = clawd_signal_dispatch(ctx);
    assert(dispatched >= 0);
...
static void test_supervisor_lifecycle(void)
{
    ...
    clawd_supervisor_t *sv = clawd_supervisor_new();
    assert(sv != NULL);

    /* Add a simple long-running process */
    char *argv[] = {"sleep", "300", NULL};
    clawd_supervised_t proc = {
        .name             = "test-sleep",
        .cmd              = "sleep",
        .argv             = argv,
    ...
    int rc = clawd_supervisor_add(sv, &proc);
    assert(rc == 0);

    /* Duplicate name should fail */
    rc = clawd_supervisor_add(sv, &proc);
    assert(rc == -1);

    /* Start should launch the process */
    rc = clawd_supervisor_start(sv);
    assert(rc == 0);
    ...
    rc = clawd_supervisor_restart(sv, "test-sleep");
    assert(rc == 0);

    usleep(100000);

    clawd_supervisor_free(sv);
...
static void test_supervisor_restart(void)
{
    ...
    rc = clawd_supervisor_restart(sv, "restartable");
    assert(rc == 0);
    usleep(100000);

    /* Restart non-existent name should fail */
    rc = clawd_supervisor_restart(sv, "nonexistent");
    assert(rc == -1);
...
int main(void)
{
    clawd_log_init("test_process", CLAWD_LOG_WARN);

    printf("\n=== libclawd-process tests ===\n\n");
