...
bool clawd_ssrf_is_private_ip(const char *host)
{
    ...
    if (rc != 0) {
        CLAWD_WARN("SSRF: getaddrinfo(%s) failed: %s",
...
{
    if (!url) {
        CLAWD_WARN("SSRF: NULL URL blocked");
        return -1;
    }

    char *host = extract_host(url);
    if (!host) {
        CLAWD_WARN("SSRF: failed to extract host from URL: %s", url);
        return -1;
    }

    /* Check explicit block list first (always wins) */
    if (is_in_block_list(host)) {
        CLAWD_WARN("SSRF: host '%s' is in block list", host);
        free(host);
        return -1;
    }
    ...
    if (is_in_allow_list(host)) {
        CLAWD_DEBUG("SSRF: host '%s' is in allow list", host);
        free(host);
        return 0;
    }

    /* Resolve and check all addresses against private ranges */
    if (clawd_ssrf_is_private_ip(host)) {
        CLAWD_WARN("SSRF: blocked request to private host '%s'", host);
        free(host);
        return -1;
    }

    CLAWD_DEBUG("SSRF: host '%s' passed validation", host);
    free(host);
    return 0;
}
