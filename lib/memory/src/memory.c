...
static int  memory_init_schema(clawd_memory_t *mem);
static int  memory_try_load_vec(clawd_memory_t *mem);
static int  memory_prepare_statements(clawd_memory_t *mem);
static void memory_finalize_statements(clawd_memory_t *mem);
...
static void mmr_rerank(clawd_memory_entry_t *entries, int count,
...
clawd_memory_t *
clawd_memory_open(const char *db_path)
{
    if (!db_path) return NULL;

    clawd_memory_t *mem = calloc(1, sizeof(*mem));
    if (!mem) return NULL;

    int rc = sqlite3_open(db_path, &mem->db);
    if (rc != SQLITE_OK) {
        CLAWD_ERROR("sqlite3_open(%s): %s", db_path, sqlite3_errmsg(mem->db));
    ...
    if (memory_init_schema(mem) != 0) {
        CLAWD_ERROR("failed to initialise memory schema");
        clawd_memory_close(mem);
        return NULL;
    }

    if (memory_prepare_statements(mem) != 0) {
        CLAWD_ERROR("failed to prepare memory statements");
        clawd_memory_close(mem);
        return NULL;
    }
...
void
clawd_memory_close(clawd_memory_t *mem)
{
    if (!mem) return;
...
int64_t
clawd_memory_add(clawd_memory_t *mem, const char *content,
                  const char *source, const char *category)
{
    ...
    if (rc != SQLITE_DONE) {
        CLAWD_ERROR("memory_add: %s", sqlite3_errmsg(mem->db));
        return -1;
    }
...
int
clawd_memory_update(clawd_memory_t *mem, int64_t id, const char *content)
{
    if (!mem || !content) return -1;
    ...
    if (rc != SQLITE_DONE) {
        CLAWD_ERROR("memory_update: %s", sqlite3_errmsg(mem->db));
        return -1;
    }
...
int
clawd_memory_delete(clawd_memory_t *mem, int64_t id)
{
    if (!mem) return -1;
    ...
    if (rc != SQLITE_DONE) {
        CLAWD_ERROR("memory_delete: %s", sqlite3_errmsg(mem->db));
        return -1;
    }
...
int
clawd_memory_get(clawd_memory_t *mem, int64_t id,
...
int
clawd_memory_search(clawd_memory_t *mem,
                     ...
                     clawd_memory_entry_t **results, int *count)
{
    ...
    *count   = 0;
     ...
     */
    clawd_memory_entry_t *candidates = NULL;
    int n_candidates = 0;
    ...
    if (opts->use_bm25 && mem->has_fts5) {
        ...
        if (sqlite3_prepare_v2(mem->db, sql, -1, &st, NULL) == SQLITE_OK) {
            ...
            while (sqlite3_step(st) == SQLITE_ROW) {
                if (n_candidates >= cap) {
                    cap *= 2;
                    clawd_memory_entry_t *tmp = realloc(candidates,
                            (size_t)cap * sizeof(*candidates));
                    if (!tmp) break;
                    candidates = tmp;
                }

                clawd_memory_entry_t *e = &candidates[n_candidates];
                memset(e, 0, sizeof(*e));
    ...
    if (opts->min_score > 0.0f && n_candidates > 0) {
        ...
        for (int i = 0; i < n_candidates; i++) {
            if (candidates[i].score >= (double)opts->min_score) {
                ...
                clawd_memory_entry_free(&candidates[i]);
            }
        }
...
void
clawd_memory_entry_free(clawd_memory_entry_t *entry)
{
    if (!entry) return;
...
void
clawd_memory_entry_array_free(clawd_memory_entry_t *entries, int count)
{
    if (!entries) return;
    for (int i = 0; i < count; i++) {
        clawd_memory_entry_free(&entries[i]);
    }
    free(entries);
}
...
static int
memory_init_schema(clawd_memory_t *mem)
{
    char *errmsg = NULL;
    ...
    if (rc != SQLITE_OK) {
        CLAWD_ERROR("create entries table: %s", errmsg);
        sqlite3_free(errmsg);
        return -1;
    }
    ...
    if (rc == SQLITE_OK) {
        mem->has_fts5 = true;
        CLAWD_DEBUG("FTS5 virtual table ready");
    } else {
        mem->has_fts5 = false;
        CLAWD_WARN("FTS5 not available: %s", errmsg);
        sqlite3_free(errmsg);
    }
    ...
    if (mem->has_vec) {
        ...
        if (rc != SQLITE_OK) {
            CLAWD_WARN("vec_entries creation failed: %s", errmsg);
            sqlite3_free(errmsg);
            mem->has_vec = false;
        } else {
            CLAWD_DEBUG("vec_entries virtual table ready");
        }
    }
...
memory_try_load_vec(clawd_memory_t *mem)
{
    ...
    for (int i = 0; vec_paths[i]; i++) {
        ...
        if (rc == SQLITE_OK) {
            mem->has_vec = true;
            CLAWD_INFO("loaded sqlite-vec from %s", vec_paths[i]);
            return 0;
        }
        if (errmsg) {
            sqlite3_free(errmsg);
...
static int
memory_prepare_statements(clawd_memory_t *mem)
{
    int rc;
...
static void
memory_finalize_statements(clawd_memory_t *mem)
{
...
static void
mmr_rerank(clawd_memory_entry_t *entries, int count, int desired, float lambda)
{
    ...
    clawd_memory_entry_t *tmp = calloc((size_t)desired, sizeof(*tmp));
    if (tmp) {
