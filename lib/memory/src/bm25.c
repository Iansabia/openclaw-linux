 ...
 */
int
clawd_bm25_score(const char *query, const char **documents, int n_docs,
                  double *scores)
{
 ...
 */
double
clawd_bm25_score_single(const char *query, const char *document)
{
    double score = 0.0;
    const char *docs[1] = { document };
    clawd_bm25_score(query, docs, 1, &score);
    return score;
}
