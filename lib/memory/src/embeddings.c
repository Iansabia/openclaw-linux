...
struct clawd_embed_ctx {
    clawd_embed_provider_t  provider;
    char                   *api_key;
    char                   *base_url;
    CURL                   *curl;
};
                      ...
                      float **embeddings, int *dim)
{
    ...
    if (!root) {
        CLAWD_ERROR("embeddings: failed to parse JSON response");
        return -1;
    }
    ...
    if (err) {
        ...
        CLAWD_ERROR("embeddings API error: %s",
    ...
    if (!cJSON_IsArray(data)) {
        CLAWD_ERROR("embeddings: 'data' is not an array");
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (n_items < 1) {
        CLAWD_ERROR("embeddings: empty data array");
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (!cJSON_IsArray(first_emb)) {
        CLAWD_ERROR("embeddings: first embedding is not an array");
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (d <= 0) {
        CLAWD_ERROR("embeddings: zero-dimension embedding");
        cJSON_Delete(root);
        return -1;
    }
...
parse_local_response(const char *json, float **embedding, int *dim)
{
    cJSON *root = cJSON_Parse(json);
    if (!root) {
        CLAWD_ERROR("embeddings: failed to parse local response");
        return -1;
    }

    cJSON *emb = cJSON_GetObjectItemCaseSensitive(root, "embedding");
    if (!cJSON_IsArray(emb)) {
        CLAWD_ERROR("embeddings: 'embedding' is not an array");
        cJSON_Delete(root);
        return -1;
    }
               ...
               const char *body, response_buf_t *resp)
{
    ...
    if (cc != CURLE_OK) {
        CLAWD_ERROR("embeddings HTTP request failed: %s",
    ...
    if (http_code < 200 || http_code >= 300) {
        CLAWD_ERROR("embeddings HTTP %ld: %.*s",
                    http_code,
...
clawd_embed_ctx_t *
clawd_embed_ctx_new(clawd_embed_provider_t provider, const char *api_key)
{
    clawd_embed_ctx_t *ctx = calloc(1, sizeof(*ctx));
    if (!ctx) return NULL;
    ...
    switch (provider) {
    case CLAWD_EMBED_OPENAI:
        ctx->base_url = embed_strdup(OPENAI_EMBED_URL);
        break;
    case CLAWD_EMBED_LOCAL:
...
void
clawd_embed_ctx_free(clawd_embed_ctx_t *ctx)
{
    if (!ctx) return;
...
int
clawd_embed_text(clawd_embed_ctx_t *ctx, const char *text,
                  float **embedding, int *dim)
{
    ...
    *dim       = 0;
    ...
    if (ctx->provider == CLAWD_EMBED_OPENAI) {
    ...
    } else if (ctx->provider == CLAWD_EMBED_LOCAL) {
...
int
clawd_embed_batch(clawd_embed_ctx_t *ctx, const char **texts, int count,
                   float **embeddings, int *dim)
{
    ...
    *dim        = 0;
    ...
    if (ctx->provider == CLAWD_EMBED_OPENAI) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            int rc = clawd_embed_text(ctx, texts[i], &emb, &edim);
            if (rc != 0) {
                free(all);
                return -1;
            }
            ...
            if (i == 0) {
                ...
                CLAWD_ERROR("embeddings: inconsistent dimensions (%d vs %d)",
                            edim, d);
                free(emb);
                free(all);
                return -1;
...
int
clawd_embed_dimension(clawd_embed_provider_t provider)
{
    switch (provider) {
    case CLAWD_EMBED_OPENAI: return OPENAI_DIM;
    case CLAWD_EMBED_LOCAL:  return LOCAL_DIM;
    default:                 return -1;
    }
}
