---
description: Update Kelpbot from upstream when branch has diverged (ahead/behind)
---

# Kelpbot Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Kelpbot" || true

# Move old version
mv /Applications/Kelpbot.app /tmp/Kelpbot-backup.app

# Install new build
cp -R dist/Kelpbot.app /Applications/

# Launch
open /Applications/Kelpbot.app
```
...
ps aux | grep -E "(kelpbot|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm kelpbot agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
morph-mcp_warpgrep_codebase_search search_string="Find deprecated FileManager.default and Thread.isMainThread usages in Swift files" repo_path="/Volumes/Main SSD/Developer/kelpis"
morph-mcp_warpgrep_codebase_search search_string="Locate Peekaboo submodule and macOS app Swift files with concurrency issues" repo_path="/Volumes/Main SSD/Developer/kelpis"
```
...
cd /Volumes/Main SSD/Developer/kelpis
...
# Update kelpbot.json with fallback chains
...
morph-mcp_warpgrep_codebase_search search_string="Find all Swift files using deprecated FileManager.default or Thread.isMainThread" repo_path="/Volumes/Main SSD/Developer/kelpis"
```
...
pnpm kelpbot agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
