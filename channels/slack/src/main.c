/*
 * clawd-linux :: clawd-channel-slack
 ...
 * clawd-gateway over a Unix domain socket (JSON-RPC), and posts the
 * assistant response back to the originating Slack channel.
 *
 * Usage: clawd-channel-slack [options]
 * Options:
...
static int tls_connect(const char *host, int port)
{
    ...
    if (fd < 0) {
        CLAWD_ERROR("tcp_connect(%s:%d) failed", host, port);
        return -1;
    }
...
static int ws_handshake(const char *host, const char *path)
{
    ...
    if (strstr(response, "101") == NULL) {
        CLAWD_ERROR("ws_handshake: no 101 response:\n%s", response);
        return -1;
    }
...
static cJSON *slack_api_call(const char *method, cJSON *body)
{
    ...
    clawd_http_header_add(&headers, "Authorization", auth);
    if (body_str)
        clawd_http_header_add(&headers, "Content-Type",
    ...
    if (ret != CLAWD_OK) {
        CLAWD_ERROR("slack_api_call(%s): HTTP request failed", method);
    ...
    bool ok = clawd_json_get_bool(root, "ok", false);
    if (!ok) {
        const char *err_str = clawd_json_get_string(root, "error");
        CLAWD_ERROR("slack_api_call(%s): %s", method, err_str ? err_str : "?");
        cJSON_Delete(root);
        return NULL;
    }
...
static char *slack_get_ws_url(void)
{
    ...
    clawd_http_header_add(&headers, "Authorization", auth);
    clawd_http_header_add(&headers, "Content-Type", "application/x-www-form-urlencoded");
    ...
    if (ret != CLAWD_OK) {
        CLAWD_ERROR("apps.connections.open failed");
        clawd_http_response_free(&resp);
        return NULL;
    }
    ...
    if (!ok) {
        const char *err_str = clawd_json_get_string(root, "error");
        CLAWD_ERROR("apps.connections.open: %s", err_str ? err_str : "?");
        cJSON_Delete(root);
        return NULL;
    }

    const char *ws_url = clawd_json_get_string(root, "url");
...
{
    const char *type = clawd_json_get_string(envelope, "type");
    const char *envelope_id = clawd_json_get_string(envelope, "envelope_id");
    ...
    if (strcmp(type, "hello") == 0) {
        CLAWD_INFO("Socket Mode: connected (hello)");
        return;
    }

    if (strcmp(type, "disconnect") == 0) {
        const char *reason = clawd_json_get_string(envelope, "reason");
        CLAWD_INFO("Socket Mode: disconnect requested (reason=%s)",
                   reason ? reason : "?");
        g_shutdown = 1;
        return;
    }
    ...
    cJSON *payload = clawd_json_get_object(envelope, "payload");
    if (!payload)
        return;

    cJSON *event = clawd_json_get_object(payload, "event");
    if (!event)
        return;

    const char *event_type = clawd_json_get_string(event, "type");
    if (!event_type || strcmp(event_type, "message") != 0)
        return;

    /* Ignore subtypes (edits, joins, etc.) */
    const char *subtype = clawd_json_get_string(event, "subtype");
    if (subtype)
        return;

    const char *text    = clawd_json_get_string(event, "text");
    const char *channel = clawd_json_get_string(event, "channel");
    const char *user    = clawd_json_get_string(event, "user");
    ...
    const char *msg = text;
    if (is_mentioned) {
        char *cleaned = clawd_str_replace(text, mention_str, "");
        if (cleaned) {
            /* Trim leading spaces */
...
static int socket_mode_loop(void)
{
    while (!g_shutdown) {
        ...
        if (opcode == WS_OP_CLOSE) {
            CLAWD_INFO("received WebSocket CLOSE");
            free(frame);
            return -1;
        }
...
int main(int argc, char *argv[])
{
    ...
    clawd_log_init("slack", log_level);
    ...
    CLAWD_INFO("clawd-channel-slack %s starting", SLACK_CHANNEL_VERSION);
    CLAWD_INFO("gateway socket: %s", g_socket_path);

    /* Verify bot token and get our user ID */
    cJSON *auth = slack_api_call("auth.test", NULL);
    if (auth) {
        const char *uid = clawd_json_get_string(auth, "user_id");
        const char *uname = clawd_json_get_string(auth, "user");
        if (uid)
            snprintf(g_bot_user_id, sizeof(g_bot_user_id), "%s", uid);
        CLAWD_INFO("authenticated as @%s (id=%s)",
                   uname ? uname : "?", g_bot_user_id);
        cJSON_Delete(auth);
    } else {
        CLAWD_ERROR("auth.test failed - check your bot token");
        clawd_http_cleanup();
        clawd_config_free(&g_cfg);
        return 1;
    }
    ...
    while (!g_shutdown) {
        ...
        if (!ws_url) {
            CLAWD_ERROR("failed to get Socket Mode URL, retrying in %ds", backoff);
            sleep(backoff);
            if (backoff < 60)
                backoff *= 2;
            continue;
        ...
        CLAWD_INFO("Socket Mode URL: %s", ws_url);
        ...
        if (parse_wss_url(ws_url, host, sizeof(host),
            ...
            CLAWD_ERROR("invalid WebSocket URL");
            free(ws_url);
            break;
        }
        free(ws_url);
        ...
        if (tls_connect(host, port) < 0) {
            tls_cleanup();
            CLAWD_ERROR("TLS connection failed, retrying in %ds", backoff);
            sleep(backoff);
            if (backoff < 60)
                backoff *= 2;
            continue;
        ...
        if (ws_handshake(host, path) < 0) {
            tls_cleanup();
            CLAWD_ERROR("WS handshake failed, retrying in %ds", backoff);
            sleep(backoff);
            if (backoff < 60)
                backoff *= 2;
            continue;
        ...
        CLAWD_INFO("reconnecting in %ds...", backoff);
        sleep(backoff);
        if (backoff < 60)
            backoff *= 2;
    }
    ...
    CLAWD_INFO("clawd-channel-slack exited cleanly");
    return 0;
}
