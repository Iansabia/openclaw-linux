...
static cJSON *tg_api_call(const char *method, cJSON *params)
{
    ...
    if (ret != CLAWD_OK) {
        CLAWD_ERROR("tg_api_call(%s): HTTP request failed", method);
        clawd_http_response_free(&resp);
        return NULL;
    }

    if (resp.status_code < 200 || resp.status_code >= 300) {
        CLAWD_ERROR("tg_api_call(%s): HTTP %d", method, resp.status_code);
        clawd_http_response_free(&resp);
        return NULL;
    }
    ...
    if (!root) {
        CLAWD_ERROR("tg_api_call(%s): invalid JSON response", method);
        return NULL;
    }

    bool ok = clawd_json_get_bool(root, "ok", false);
    if (!ok) {
        const char *desc = clawd_json_get_string(root, "description");
        CLAWD_ERROR("tg_api_call(%s): error: %s", method, desc ? desc : "?");
        cJSON_Delete(root);
        return NULL;
    }
                          ...
                          const char *user_id)
{
    ...
    if (connect(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        CLAWD_ERROR("connect(%s): %s", g_socket_path, strerror(errno));
        close(fd);
        return NULL;
    }
...
static void handle_update(cJSON *update)
{
    ...
    int64_t update_id = (int64_t)clawd_json_get_int(update, "update_id", 0);
    if (update_id >= g_update_offset)
        g_update_offset = update_id + 1;

    cJSON *message = clawd_json_get_object(update, "message");
    if (!message)
        return;

    const char *text = clawd_json_get_string(message, "text");
    if (!text || *text == '\0')
        return;

    cJSON *chat = clawd_json_get_object(message, "chat");
    if (!chat)
        return;

    int64_t chat_id = (int64_t)clawd_json_get_int(chat, "id", 0);
    if (chat_id == 0)
        return;

    const char *chat_type = clawd_json_get_string(chat, "type");

    cJSON *from = clawd_json_get_object(message, "from");
    const char *username = from ? clawd_json_get_string(from, "username") : NULL;
    const char *first_name = from ? clawd_json_get_string(from, "first_name") : NULL;
    bool is_bot = from ? clawd_json_get_bool(from, "is_bot", false) : false;
     ...
     *   1. Messages starting with /clawd
    ...
    if (!is_private) {
        if (clawd_str_starts_with(text, "/clawd")) {
            msg = text + 6;  /* strlen("/clawd") */
            if (*msg == '@')  /* skip /clawd@botname */
            ...
            if (strstr(text, mention)) {
                /* Strip the mention */
                char *cleaned = clawd_str_replace(text, mention, "");
                if (cleaned) {
    ...
    CLAWD_INFO("message from %s (%s) in chat %lld: %.80s%s",
...
{
    CLAWD_INFO("starting long-poll loop (timeout=%ds)", POLL_TIMEOUT_SEC);
    ...
    while (!g_shutdown) {
        ...
        if (!updates) {
            if (g_shutdown)
                break;
            CLAWD_WARN("getUpdates failed, retrying in 5s...");
            sleep(5);
            continue;
        }
        ...
        if (count > 0)
            CLAWD_DEBUG("received %d update(s)", count);
...
int main(int argc, char *argv[])
{
    ...
    clawd_log_init("telegram", log_level);
    ...
    CLAWD_INFO("clawd-channel-telegram %s starting", TELEGRAM_CHANNEL_VERSION);
    CLAWD_INFO("gateway socket: %s", g_socket_path);

    /* Verify bot token by calling getMe */
    cJSON *me = tg_api_call("getMe", NULL);
    if (me) {
        const char *uname = clawd_json_get_string(me, "username");
        const char *fname = clawd_json_get_string(me, "first_name");
        if (uname)
            snprintf(g_bot_username, sizeof(g_bot_username), "%s", uname);
        CLAWD_INFO("logged in as @%s (%s)",
                   uname ? uname : "?", fname ? fname : "?");
        cJSON_Delete(me);
    } else {
        CLAWD_ERROR("getMe failed - check your bot token");
        clawd_http_cleanup();
        clawd_config_free(&g_cfg);
        return 1;
    }
    ...
    CLAWD_INFO("clawd-channel-telegram exited cleanly");
    return 0;
}
