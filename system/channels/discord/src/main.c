...
static const char           *g_bot_prefix     = "!kelp";
...
static int tcp_connect(const char *host, int port)
{
    ...
    if (fd < 0)
        KELP_ERROR("tcp_connect(%s:%d): all addresses failed", host, port);
...
static int ws_handshake(const char *host, const char *path)
{
    ...
    if (SSL_write(g_ssl, request, req_len) <= 0) {
        KELP_ERROR("ws_handshake: failed to send upgrade request");
        return -1;
    }
    ...
    if (strstr(response, "101") == NULL) {
        KELP_ERROR("ws_handshake: did not get 101 response:\n%s", response);
        return -1;
    }

    KELP_DEBUG("WebSocket handshake complete");
    return 0;
}
...
static int gw_send_identify(void)
{
    ...
    cJSON_AddStringToObject(props, "browser", "kelp");
    cJSON_AddStringToObject(props, "device", "kelp");

    int ret = gw_send_json(obj);
    cJSON_Delete(obj);

    KELP_INFO("sent IDENTIFY");
    return ret;
}
...
static int gw_send_resume(void)
{
    ...
    KELP_INFO("sent RESUME (session=%s, seq=%d)", g_session_id, g_last_seq);
    return ret;
}
...
{
    (void)arg;
    KELP_DEBUG("heartbeat thread started (interval=%d ms)",
    ...
    while (!g_shutdown && g_heartbeat_running) {
        if (!g_heartbeat_ack) {
            KELP_WARN("heartbeat ACK not received, connection may be zombied");
        }
    ...
    KELP_DEBUG("heartbeat thread exiting");
    return NULL;
}
...
/* ---- Gateway Unix socket (JSON-RPC to kelp-gateway) -------------------- */
...
static int gateway_connect_unix(const char *path)
{
    ...
    if (connect(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        KELP_ERROR("connect(%s): %s", path, strerror(errno));
        close(fd);
        return -1;
    }
...
/**
 * Send a chat message to kelp-gateway via JSON-RPC and return the response.
...
static int discord_send_message(const char *channel_id, const char *content)
{
    ...
    kelp_http_header_add(&headers, "Authorization", auth_header);
    kelp_http_header_add(&headers, "Content-Type", "application/json");
    kelp_http_header_add(&headers, "User-Agent", "kelp-channel-discord/0.1.0");
    ...
    if (ret != KELP_OK) {
        KELP_ERROR("discord REST: HTTP request failed");
    } else if (resp.status_code < 200 || resp.status_code >= 300) {
        KELP_ERROR("discord REST: HTTP %d: %.*s",
                    resp.status_code,
                    (int)(resp.body_len > 200 ? 200 : resp.body_len),
                    resp.body);
    } else {
        KELP_DEBUG("discord REST: message sent to channel %s", channel_id);
    }

    kelp_http_response_free(&resp);
    kelp_http_header_free(headers);
    free(body_str);
    return (ret == KELP_OK && resp.status_code >= 200 && resp.status_code < 300)
               ? 0 : -1;
}
...
static void *async_message_handler(void *arg)
{
    ...
    KELP_DEBUG("async handler: processing message from %s", ctx->username);
...
{
    const char *content    = kelp_json_get_string(data, "content");
    const char *channel_id = kelp_json_get_string(data, "channel_id");

    cJSON *author = kelp_json_get_object(data, "author");
    if (!author || !content || !channel_id)
        return;

    const char *author_id  = kelp_json_get_string(author, "id");
    const char *username   = kelp_json_get_string(author, "username");
    bool is_bot = kelp_json_get_bool(author, "bot", false);
     ...
     *   1. Starts with the bot prefix (e.g. "!kelp")
     *   2. Mentions the bot user
     *   3. Is a DM (no guild_id)
     */
    const char *guild_id = kelp_json_get_string(data, "guild_id");
    bool is_dm = (guild_id == NULL);

    /* Check for bot mention in the mentions array */
    bool is_mentioned = false;
    cJSON *mentions = kelp_json_get_array(data, "mentions");
    if (mentions) {
        cJSON *m = NULL;
        cJSON_ArrayForEach(m, mentions) {
            const char *mid = kelp_json_get_string(m, "id");
    ...
    bool has_prefix = kelp_str_starts_with(content, g_bot_prefix);
    ...
    if (has_prefix) {
        ...
        if (kelp_str_starts_with(msg, mention_str)) {
    ...
    KELP_INFO("message from %s in channel %s: %.80s%s",
    ...
    if (!ctx) {
        KELP_ERROR("failed to allocate async context");
        return;
    }
    ...
    if (pthread_create(&tid, &attr, async_message_handler, ctx) != 0) {
        KELP_ERROR("failed to create async message handler thread");
        free(ctx->message);
        free(ctx->channel_id);
        free(ctx->author_id);
        free(ctx->username);
...
static void handle_gateway_event(const char *event_name, cJSON *data)
{
    if (strcmp(event_name, "READY") == 0) {
        const char *sid = kelp_json_get_string(data, "session_id");
        if (sid) {
            snprintf(g_session_id, sizeof(g_session_id), "%s", sid);
            KELP_INFO("READY: session_id=%s", g_session_id);
        }

        /* Extract our own user ID */
        cJSON *user = kelp_json_get_object(data, "user");
        if (user) {
            const char *uid = kelp_json_get_string(user, "id");
            const char *uname = kelp_json_get_string(user, "username");
            if (uid)
                snprintf(g_bot_user_id, sizeof(g_bot_user_id), "%s", uid);
            KELP_INFO("logged in as %s (id=%s)",
                       uname ? uname : "?", g_bot_user_id);
        }
    } else if (strcmp(event_name, "RESUMED") == 0) {
        KELP_INFO("session resumed");
    } else if (strcmp(event_name, "MESSAGE_CREATE") == 0) {
        handle_message_create(data);
    } else if (strcmp(event_name, "GUILD_CREATE") == 0) {
        const char *name = kelp_json_get_string(data, "name");
        KELP_INFO("joined guild: %s", name ? name : "(unknown)");
    } else {
        KELP_DEBUG("unhandled event: %s", event_name);
    }
}
...
static int gateway_loop(void)
{
    while (!g_shutdown) {
        ...
        if (opcode == WS_OP_CLOSE) {
            KELP_INFO("received WebSocket CLOSE frame");
            free(frame);
            return -1;
        }
        ...
        int op = kelp_json_get_int(msg, "op", -1);
        ...
        switch (op) {
        case GW_OP_DISPATCH: {
            const char *t = kelp_json_get_string(msg, "t");
        ...
        case GW_OP_HELLO: {
            cJSON *d = cJSON_GetObjectItem(msg, "d");
            int interval = kelp_json_get_int(d, "heartbeat_interval", 45000);
            KELP_INFO("HELLO: heartbeat_interval=%d ms", interval);
        ...
        case GW_OP_RECONNECT:
            KELP_INFO("server requested RECONNECT");
            cJSON_Delete(msg);
            return -1;
        ...
        case GW_OP_INVALID_SESSION: {
            ...
            KELP_WARN("INVALID_SESSION (resumable=%s)",
        ...
        default:
            KELP_DEBUG("unknown opcode: %d", op);
            break;
        }
...
{
    KELP_INFO("connecting to Discord Gateway...");

    if (tls_connect(DISCORD_WS_HOST, DISCORD_WS_PORT) < 0) {
        KELP_ERROR("TLS connection to Discord failed");
        return -1;
    }

    if (ws_handshake(DISCORD_WS_HOST, DISCORD_WS_PATH) < 0) {
        KELP_ERROR("WebSocket handshake failed");
        tls_cleanup();
        return -1;
    }

    KELP_INFO("connected to Discord Gateway");
    return 0;
}
...
static void disconnect_from_discord(void)
{
    ...
    KELP_INFO("disconnected from Discord");
}
...
static void print_usage(void)
{
    fprintf(stderr,
        ...
        "  -s, --socket <path>   Gateway Unix socket path\n"
        "  -P, --prefix <str>    Bot command prefix (default: !kelp)\n"
        "  -v, --verbose         Increase verbosity\n"
        "  -h, --help            Help\n",
...
int main(int argc, char *argv[])
{
    ...
    kelp_log_init("discord", log_level);
    ...
    KELP_INFO("kelp-channel-discord %s starting", DISCORD_CHANNEL_VERSION);
    KELP_INFO("gateway socket: %s", g_socket_path);
    KELP_INFO("bot prefix: \"%s\"", g_bot_prefix);
    ...
    while (!g_shutdown) {
        if (connect_to_discord() == 0) {
            ...
            KELP_INFO("will reconnect in %d seconds...", backoff);
        } else {
            KELP_ERROR("connection failed, retrying in %d seconds...", backoff);
        }
    ...
    KELP_INFO("kelp-channel-discord exited cleanly");
    return 0;
}
