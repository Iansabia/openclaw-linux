...
typedef struct {
    /* Configuration (copied from kelp_supervised_t) */
    char         *name;
    char         *cmd;
...
static int spawn_entry(sv_entry_t *e)
{
    ...
    if (pid < 0) {
        KELP_ERROR("supervisor: failed to spawn '%s': %s",
    ...
    KELP_INFO("supervisor: started '%s' (pid %d)", e->name, (int)pid);
    return 0;
}
...
static void stop_entry(sv_entry_t *e)
{
    ...
    KELP_INFO("supervisor: stopping '%s' (pid %d)", e->name, (int)e->pid);

    /* Send SIGTERM to the process group */
    kelp_proc_kill_tree(e->pid, SIGTERM);
    ...
    /* Still alive -- SIGKILL */
    KELP_WARN("supervisor: '%s' did not exit, sending SIGKILL", e->name);
    kelp_proc_kill_tree(e->pid, SIGKILL);
 ...
 */
static void check_children(kelp_supervisor_t *sv)
{
    for (int i = 0; i < sv->count; i++) {
        ...
        if (e->state == SV_STATE_RUNNING && e->pid > 0) {
            ...
            if (w > 0) {
                ...
                KELP_WARN("supervisor: '%s' (pid %d) exited with code %d",
                ...
                if (e->max_restarts > 0 &&
                    ...
                    KELP_ERROR("supervisor: '%s' reached max restarts (%d)",
                ...
                KELP_INFO("supervisor: will restart '%s' in %d ms "
                           "(attempt %d)",
        ...
        if (e->state == SV_STATE_BACKOFF) {
            ...
            if (timespec_le(&e->next_restart, &now)) {
                KELP_INFO("supervisor: restarting '%s'", e->name);
                if (spawn_entry(e) < 0) {
                    e->state = SV_STATE_FATAL;
                }
            }
...
{
    kelp_supervisor_t *sv = calloc(1, sizeof(*sv));
    if (!sv)
        return NULL;

    KELP_TRACE("supervisor created");
    return sv;
}
...
int kelp_supervisor_add(kelp_supervisor_t *sv, const kelp_supervised_t *proc)
{
    ...
    if (sv->count >= SV_MAX_ENTRIES) {
        KELP_ERROR("supervisor: max entries (%d) reached", SV_MAX_ENTRIES);
        return -1;
    }
    ...
    for (int i = 0; i < sv->count; i++) {
        if (strcmp(sv->entries[i].name, proc->name) == 0) {
            KELP_ERROR("supervisor: duplicate name '%s'", proc->name);
            return -1;
        }
    }
    ...
    sv->count++;
    KELP_INFO("supervisor: added '%s' (%s)", e->name, e->cmd);
    return 0;
}
...
int kelp_supervisor_start(kelp_supervisor_t *sv)
{
    ...
    KELP_INFO("supervisor: started %d/%d processes",
...
void kelp_supervisor_stop(kelp_supervisor_t *sv)
{
    ...
    KELP_INFO("supervisor: all processes stopped");
}
...
int kelp_supervisor_restart(kelp_supervisor_t *sv, const char *name)
{
    ...
    KELP_ERROR("supervisor: process '%s' not found", name);
    return -1;
}
