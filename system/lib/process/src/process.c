 ...
 */
static void child_setup(const kelp_proc_opts_t *opts,
 ...
 */
static void reap_child(pid_t pid, kelp_proc_result_t *result)
{
    int status = 0;
    pid_t w;
...
int kelp_proc_exec(const kelp_proc_opts_t *opts, kelp_proc_result_t *result)
{
    ...
    if (opts->use_pty) {
        if (kelp_pty_open(&pty) < 0) {
            KELP_ERROR("pty open failed: %s", strerror(errno));
            return -1;
        }
    } else {
        ...
        if (opts->stdin_data && opts->stdin_len > 0) {
            if (pipe(stdin_pipe) < 0) {
                KELP_ERROR("pipe(stdin) failed: %s", strerror(errno));
                return -1;
            }
        ...
        if (opts->capture_stdout) {
            if (pipe(stdout_pipe) < 0) {
                KELP_ERROR("pipe(stdout) failed: %s", strerror(errno));
                goto err_close;
            }
        ...
        if (opts->capture_stderr && !opts->merge_stderr) {
            if (pipe(stderr_pipe) < 0) {
                KELP_ERROR("pipe(stderr) failed: %s", strerror(errno));
                goto err_close;
            }
    ...
    if (opts->use_pty) {
        pid = kelp_pty_fork(&pty, opts->cmd, opts->argv);
        if (pid < 0) {
            KELP_ERROR("pty_fork failed: %s", strerror(errno));
            kelp_pty_close(&pty);
            return -1;
        }
        if (pid == 0) {
            /* child -- kelp_pty_fork already exec'd */
        ...
        pid = fork();
        if (pid < 0) {
            KELP_ERROR("fork failed: %s", strerror(errno));
            goto err_close;
        }
    ...
    if (drain_rc == 1) {
        ...
        KELP_WARN("process %d timed out after %d ms", (int)pid,
                   opts->timeout_ms);
    ...
    if (opts->use_pty)       kelp_pty_close(&pty);
