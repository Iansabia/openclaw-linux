...
#ifndef KELP_MAX_SIGNALS
#define KELP_MAX_SIGNALS 64
#endif

/* ---- per-signal registration -------------------------------------------- */

typedef struct {
    kelp_signal_handler_t handler;
    void                  *userdata;
    bool                   active;
} signal_slot_t;
...
struct kelp_signal_ctx {
    signal_slot_t  slots[KELP_MAX_SIGNALS];
 ...
 */
static kelp_signal_ctx_t *g_active_ctx = NULL;

/* Saved original signal actions so we can restore them on cleanup. */
static struct sigaction g_orig_actions[KELP_MAX_SIGNALS];
static bool            g_orig_saved[KELP_MAX_SIGNALS];
...
{
    kelp_signal_ctx_t *ctx = g_active_ctx;
...
{
    kelp_signal_ctx_t *ctx = calloc(1, sizeof(*ctx));
    if (!ctx)
        return NULL;
    ...
    KELP_TRACE("signal context created");
    return ctx;
}
...
void kelp_signal_ctx_free(kelp_signal_ctx_t *ctx)
{
    ...
    for (int i = 0; i < KELP_MAX_SIGNALS; i++) {
    ...
    KELP_TRACE("signal context freed");
    free(ctx);
}
                       ...
                       kelp_signal_handler_t handler, void *userdata)
{
    if (!ctx || !handler)
        return -1;
    if (signo < 1 || signo >= KELP_MAX_SIGNALS)
        return -1;
    ...
    if (sigprocmask(SIG_BLOCK, &ctx->mask, NULL) < 0) {
        KELP_ERROR("sigprocmask failed: %s", strerror(errno));
        return -1;
    }
    ...
    if (new_sfd < 0) {
        KELP_ERROR("signalfd failed: %s", strerror(errno));
        return -1;
    }
    ctx->sfd = new_sfd;
    ...
    if (sigaction(signo, &sa, NULL) < 0) {
        KELP_ERROR("sigaction(%d) failed: %s", signo, strerror(errno));
        return -1;
    }
#endif

    KELP_TRACE("watching signal %d", signo);
    return 0;
}
...
int kelp_signal_dispatch(kelp_signal_ctx_t *ctx)
{
    ...
    for (;;) {
        ...
        if (signo >= 0 && signo < KELP_MAX_SIGNALS &&
