...
extern int    kelp_bm25_score(const char *query, const char **documents,
                                int n_docs, double *scores);
extern double kelp_bm25_score_single(const char *query,
                                       const char *document);
...
test_open_close(void)
{
    ...
    kelp_memory_t *mem = kelp_memory_open(":memory:");
...
test_add_get(void)
{
    ...
    int64_t id1 = kelp_memory_add(mem,
        "The quick brown fox jumps over the lazy dog.",
        "/tmp/test.txt", "doc");
    TEST_ASSERT(id1 > 0);

    int64_t id2 = kelp_memory_add(mem,
    ...
    int rc = kelp_memory_get(mem, id1, &entry);
    TEST_ASSERT(rc == 0);
    ...
    kelp_memory_entry_free(&entry);

    /* Retrieve the second entry. */
    rc = kelp_memory_get(mem, id2, &entry);
    TEST_ASSERT(rc == 0);
    ...
    rc = kelp_memory_get(mem, 99999, &entry);
    TEST_ASSERT(rc == -1);
...
test_update(void)
{
    ...
    int64_t id = kelp_memory_add(mem, "original content", "user", "note");
    TEST_ASSERT(id > 0);

    int rc = kelp_memory_update(mem, id, "updated content");
    TEST_ASSERT(rc == 0);

    kelp_memory_entry_t entry;
    rc = kelp_memory_get(mem, id, &entry);
    TEST_ASSERT(rc == 0);
    TEST_ASSERT(strcmp(entry.content, "updated content") == 0);
    kelp_memory_entry_free(&entry);

    /* Update non-existent id should fail. */
    rc = kelp_memory_update(mem, 99999, "something");
    TEST_ASSERT(rc == -1);
...
test_delete(void)
{
    ...
    int64_t id = kelp_memory_add(mem, "to be deleted", "user", "note");
    TEST_ASSERT(id > 0);

    int rc = kelp_memory_delete(mem, id);
    TEST_ASSERT(rc == 0);
...
test_search_bm25(void)
{
    ...
    kelp_memory_entry_t *results = NULL;
    int count = 0;
    int rc = kelp_memory_search(mem, &opts, &results, &count);
    TEST_ASSERT(rc == 0);
    TEST_ASSERT(count > 0);
    ...
    kelp_memory_entry_array_free(results, count);
    ...
    results = NULL;
    count   = 0;
    rc = kelp_memory_search(mem, &opts, &results, &count);
    TEST_ASSERT(rc == 0);
...
test_bm25_scoring(void)
{
    ...
    double scores[4];
    int rc = kelp_bm25_score("fox", docs, 4, scores);
    TEST_ASSERT(rc == 0);
    ...
    double s = kelp_bm25_score_single("fox", docs[0]);
    TEST_ASSERT(s > 0.0);
...
test_embed_dimension(void)
{
    ...
    int d = kelp_embed_dimension(KELP_EMBED_OPENAI);
    TEST_ASSERT(d == 1536);

    d = kelp_embed_dimension(KELP_EMBED_LOCAL);
    TEST_ASSERT(d == 384);
...
test_embed_ctx_lifecycle(void)
{
    ...
    kelp_embed_ctx_t *ctx = kelp_embed_ctx_new(KELP_EMBED_OPENAI,
                                                  "sk-test-dummy-key");
    TEST_ASSERT(ctx != NULL);
    kelp_embed_ctx_free(ctx);

    /* Local provider with NULL key. */
    ctx = kelp_embed_ctx_new(KELP_EMBED_LOCAL, NULL);
    TEST_ASSERT(ctx != NULL);
    kelp_embed_ctx_free(ctx);

    /* Free NULL should be safe. */
    kelp_embed_ctx_free(NULL);
...
test_watcher_lifecycle(void)
{
    ...
    int fd = kelp_watcher_fd(w);
    TEST_ASSERT(fd >= 0);

    kelp_watcher_free(w);

    /* Free NULL should be safe. */
    kelp_watcher_free(NULL);
...
test_watcher_add_remove(void)
{
    ...
    int rc = kelp_watcher_add(w, "/tmp", KELP_WATCH_ALL, false);
    TEST_ASSERT(rc == 0);

    /* Remove it. */
    rc = kelp_watcher_remove(w, "/tmp");
    TEST_ASSERT(rc == 0);
...
test_entry_free_safety(void)
{
    ...
    /* Should not crash. */
    kelp_memory_entry_free(&entry);
    kelp_memory_entry_free(NULL);
    kelp_memory_entry_array_free(NULL, 0);
...
main(void)
{
    printf("libkelp-memory :: test suite\n");
    printf("====================================\n\n");
