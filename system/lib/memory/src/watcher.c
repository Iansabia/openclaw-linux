...
static int   watcher_add_single(kelp_watcher_t *w, const char *path,
                                 kelp_watch_event_t events);
static int   watcher_add_recursive(kelp_watcher_t *w, const char *path,
                                    kelp_watch_event_t events);
static watch_entry_t *watcher_find_by_wd(kelp_watcher_t *w, int wd);
static watch_entry_t *watcher_find_by_path(kelp_watcher_t *w,
                                            const char *path);
...
kelp_watcher_t *
kelp_watcher_new(void)
{
    kelp_watcher_t *w = calloc(1, sizeof(*w));
    if (!w) return NULL;
    ...
    if (w->fd < 0) {
        KELP_ERROR("inotify_init1: %s", strerror(errno));
        free(w);
        return NULL;
    }
...
#else
    KELP_ERROR("file watching not supported on this platform");
    free(w);
    return NULL;
#endif
    ...
    if (pipe(w->pipe_fds) != 0) {
        KELP_ERROR("pipe: %s", strerror(errno));
        close(w->fd);
        free(w);
        return NULL;
    }
...
void
kelp_watcher_free(kelp_watcher_t *w)
{
    if (!w) return;
...
int
kelp_watcher_add(kelp_watcher_t *w, const char *path,
...
int
kelp_watcher_remove(kelp_watcher_t *w, const char *path)
{
    if (!w || !path) return -1;
...
int
kelp_watcher_start(kelp_watcher_t *w, kelp_watch_cb cb, void *userdata)
{
    if (!w || !cb) return -1;
    ...
    if (rc != 0) {
        KELP_ERROR("pthread_create: %s", strerror(rc));
        w->running = 0;
        return -1;
    }
...
void
kelp_watcher_stop(kelp_watcher_t *w)
{
...
int
kelp_watcher_fd(kelp_watcher_t *w)
{
    if (!w) return -1;
    return w->fd;
}
...
watcher_thread_func(void *arg)
{
    ...
    while (w->running) {
        ...
        while (ptr < buf + n) {
            ...
            kelp_watch_event_t event = 0;
            if (ev->mask & IN_CREATE)     event |= KELP_WATCH_CREATE;
            if (ev->mask & IN_MODIFY)     event |= KELP_WATCH_MODIFY;
            if (ev->mask & IN_DELETE)     event |= KELP_WATCH_DELETE;
            if (ev->mask & IN_MOVED_FROM) event |= KELP_WATCH_MOVE;
            if (ev->mask & IN_MOVED_TO)   event |= KELP_WATCH_MOVE;
...
watcher_thread_func(void *arg)
{
    ...
    while (w->running) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            kelp_watch_event_t ev = 0;
            if (events[i].fflags & NOTE_WRITE)  ev |= KELP_WATCH_MODIFY;
            if (events[i].fflags & NOTE_DELETE)  ev |= KELP_WATCH_DELETE;
            if (events[i].fflags & NOTE_RENAME)  ev |= KELP_WATCH_MOVE;
            if (events[i].fflags & NOTE_EXTEND)  ev |= KELP_WATCH_MODIFY;
            /* NOTE: kqueue does not have a direct CREATE event on the
             * watched fd itself.  We treat NOTE_LINK as CREATE. */
            if (events[i].fflags & NOTE_LINK)    ev |= KELP_WATCH_CREATE;
...
static int
watcher_add_single(kelp_watcher_t *w, const char *path,
                    kelp_watch_event_t events)
{
...
#ifdef __linux__
    uint32_t mask = 0;
    if (events & KELP_WATCH_CREATE) mask |= IN_CREATE;
    if (events & KELP_WATCH_MODIFY) mask |= IN_MODIFY;
    if (events & KELP_WATCH_DELETE) mask |= IN_DELETE;
    if (events & KELP_WATCH_MOVE)   mask |= IN_MOVED_FROM | IN_MOVED_TO;

    wd = inotify_add_watch(w->fd, path, mask);
    if (wd < 0) {
        KELP_WARN("inotify_add_watch(%s): %s", path, strerror(errno));
        return -1;
    }
    ...
    if (wd < 0) {
        KELP_WARN("open(%s, O_EVTONLY): %s", path, strerror(errno));
        return -1;
    }

    unsigned int fflags = 0;
    if (events & KELP_WATCH_CREATE) fflags |= NOTE_LINK;
    if (events & KELP_WATCH_MODIFY) fflags |= NOTE_WRITE | NOTE_EXTEND;
    if (events & KELP_WATCH_DELETE) fflags |= NOTE_DELETE;
    if (events & KELP_WATCH_MOVE)   fflags |= NOTE_RENAME;

    struct kevent change;
    EV_SET(&change, (uintptr_t)wd, EVFILT_VNODE,
           EV_ADD | EV_CLEAR, fflags, 0, NULL);
    if (kevent(w->fd, &change, 1, NULL, 0, NULL) < 0) {
        KELP_WARN("kevent register(%s): %s", path, strerror(errno));
        close(wd);
        return -1;
    }
#else
    (void)events;
    KELP_ERROR("file watching not supported");
    return -1;
#endif
...
static int
watcher_add_recursive(kelp_watcher_t *w, const char *path,
                       kelp_watch_event_t events)
{
...
watcher_find_by_wd(kelp_watcher_t *w, int wd)
{
...
static watch_entry_t *
watcher_find_by_path(kelp_watcher_t *w, const char *path)
{
    for (watch_entry_t *e = w->entries; e; e = e->next) {
