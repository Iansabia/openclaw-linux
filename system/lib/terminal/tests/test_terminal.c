...
static void test_ansi_strip_plain(void)
{
    ...
    char *out = kelp_ansi_strip("hello world");
    assert(out != NULL);
...
static void test_ansi_strip_color(void)
{
    ...
    char *out = kelp_ansi_strip("\033[31mred\033[0m");
    assert(out != NULL);
...
static void test_ansi_strip_complex(void)
{
    ...
    char *out = kelp_ansi_strip("\033[1;4;31mhello\033[0m \033[32mworld\033[0m");
    assert(out != NULL);
    assert(strcmp(out, "hello world") == 0);
    free(out);
...
static void test_ansi_strip_cursor(void)
{
    ...
    char *out = kelp_ansi_strip("before\033[3Aafter");
    assert(out != NULL);
...
static void test_ansi_strip_osc(void)
{
    ...
    char *out = kelp_ansi_strip("\033]0;title\007visible");
    assert(out != NULL);
...
static void test_ansi_strip_osc_st(void)
{
    ...
    char *out = kelp_ansi_strip("\033]0;title\033\\visible");
    assert(out != NULL);
    assert(strcmp(out, "visible") == 0);
    free(out);
...
static void test_ansi_strip_256color(void)
{
    ...
    char *out = kelp_ansi_strip("\033[38;5;196mred256\033[0m");
    assert(out != NULL);
...
static void test_ansi_strip_truecolor(void)
{
    ...
    char *out = kelp_ansi_strip("\033[38;2;255;0;0mtrue\033[0m");
    assert(out != NULL);
...
static void test_ansi_strlen_complex(void)
{
    ...
    assert(kelp_ansi_strlen("\033[1;4;33mtest\033[0m") == 4);
...
static void test_ansi_strlen_ascii(void)
{
    ...
    assert(kelp_ansi_strlen("abcdef") == 6);
    assert(kelp_ansi_strlen("A") == 1);
    assert(kelp_ansi_strlen("  ") == 2);
...
static void test_ansi_strlen_cjk(void)
{
     ...
     */
    assert(kelp_ansi_strlen("\xe4\xbd\xa0\xe5\xa5\xbd") == 4);
...
static void test_ansi_strlen_cjk_mixed(void)
{
     ...
     */
    assert(kelp_ansi_strlen("hi\xe4\xbd\xa0\xe5\xa5\xbd") == 6);
...
static void test_ansi_strlen_emoji(void)
{
     ...
     */
    size_t w = kelp_ansi_strlen("\xf0\x9f\x98\x80");
    assert(w >= 1 && w <= 2);
...
static void test_ansi_strlen_cjk_with_ansi(void)
{
     ...
     */
    assert(kelp_ansi_strlen("\033[31m\xe4\xbd\xa0\xe5\xa5\xbd\033[0m") == 4);
...
static void test_table_basic(void)
{
    ...
    kelp_table_set_header(t, "Name", "Age", "City");
    kelp_table_add_row(t, "Alice", "30", "London");
    kelp_table_add_row(t, "Bob", "25", "Paris");

    char *s = kelp_table_render_string(t);
    assert(s != NULL);
...
{
    kelp_table_t *t = kelp_table_new(2);
    assert(t != NULL);

    kelp_table_set_header(t, "Item", "Price");
    kelp_table_set_align(t, 1, KELP_TABLE_ALIGN_RIGHT);
    kelp_table_add_row(t, "Apple", "1.50");
    kelp_table_add_row(t, "Banana", "0.75");
...
static void test_table_row_array(void)
{
    ...
    kelp_table_set_header(t, "Key", "Value");
    const char *row1[] = {"foo", "bar"};
    const char *row2[] = {"baz", "qux"};
    kelp_table_add_row_array(t, row1, 2);
    kelp_table_add_row_array(t, row2, 2);
...
static void test_progress_create(void)
{
    ...
    kelp_progress_t *p = kelp_progress_new(fp, 100, 40);
    assert(p != NULL);

    kelp_progress_update(p, 50);
    kelp_progress_finish(p);
    kelp_progress_free(p);
...
{
    kelp_progress_t *p = kelp_progress_new(NULL, 100, 40);
    assert(p == NULL);

    p = kelp_progress_new(stderr, 0, 40);
    assert(p == NULL);

    p = kelp_progress_new(stderr, 100, 0);
    assert(p == NULL);
...
static void test_spinner_create(void)
{
    ...
    kelp_spinner_t *s = kelp_spinner_new(fp, "loading...");
    assert(s != NULL);
...
{
    kelp_spinner_t *s = kelp_spinner_new(NULL, "test");
    assert(s == NULL);
...
int main(void)
{
    ...
    printf("\n=== All libkelp-terminal tests passed ===\n");
    return 0;
}
