...
static void test_ssrf_loopback_ipv4(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("127.0.0.1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("127.0.0.2"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("127.255.255.255"));
    PASS();
}
...
static void test_ssrf_private_class_a(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("10.0.0.1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("10.255.255.255"));
    PASS();
}
...
static void test_ssrf_private_class_b(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("172.16.0.1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("172.31.255.255"));
    /* 172.32.x.x is NOT private */
    ASSERT_FALSE(kelp_ssrf_is_private_ip("172.32.0.1"));
    PASS();
}
...
static void test_ssrf_private_class_c(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("192.168.0.1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("192.168.255.255"));
    PASS();
}
...
static void test_ssrf_link_local(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("169.254.0.1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("169.254.169.254")); /* metadata */
    ASSERT_TRUE(kelp_ssrf_is_private_ip("169.254.255.255"));
    PASS();
}
...
static void test_ssrf_metadata_endpoint(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("169.254.169.254"));
    PASS();
}
...
static void test_ssrf_ipv6_loopback(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("::1"));
    PASS();
}
...
static void test_ssrf_ipv6_unique_local(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("fc00::1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("fd00::1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("fdff:ffff::1"));
    PASS();
}
...
static void test_ssrf_ipv6_link_local(void)
{
    ...
    ASSERT_TRUE(kelp_ssrf_is_private_ip("fe80::1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("fe80::ffff:ffff:ffff:ffff"));
    PASS();
}
...
{
    TEST(ssrf_public_ip);
    ASSERT_FALSE(kelp_ssrf_is_private_ip("8.8.8.8"));
    ASSERT_FALSE(kelp_ssrf_is_private_ip("1.1.1.1"));
    ASSERT_FALSE(kelp_ssrf_is_private_ip("93.184.216.34"));
    PASS();
}
...
static void test_ssrf_public_ipv6(void)
{
    ...
    ASSERT_FALSE(kelp_ssrf_is_private_ip("2001:4860:4860::8888"));
    ASSERT_FALSE(kelp_ssrf_is_private_ip("2606:4700:4700::1111"));
    PASS();
}
...
{
    TEST(ssrf_null_host);
    ASSERT_TRUE(kelp_ssrf_is_private_ip(NULL));
    PASS();
}
...
static void test_ssrf_check_private_url(void)
{
    ...
    ASSERT_EQ_INT(kelp_ssrf_check("http://127.0.0.1/admin"), -1);
    ASSERT_EQ_INT(kelp_ssrf_check("http://10.0.0.1:8080/api"), -1);
    ASSERT_EQ_INT(kelp_ssrf_check("https://192.168.1.1/"), -1);
    ASSERT_EQ_INT(kelp_ssrf_check("http://169.254.169.254/latest/meta-data"), -1);
    PASS();
}
...
static void test_ssrf_check_null_url(void)
{
    ...
    ASSERT_EQ_INT(kelp_ssrf_check(NULL), -1);
    PASS();
}
...
static void test_ssrf_check_bad_url(void)
{
    ...
    ASSERT_EQ_INT(kelp_ssrf_check("not-a-url"), -1);
    ASSERT_EQ_INT(kelp_ssrf_check(""), -1);
    PASS();
}
...
static void test_ssrf_allow_list(void)
{
    ...
    ASSERT_EQ_INT(kelp_ssrf_check("http://127.0.0.1/health"), -1);

    /* Add to allow list */
    kelp_ssrf_allow_list_add("127.0.0.1");
    ASSERT_EQ_INT(kelp_ssrf_check("http://127.0.0.1/health"), 0);
...
static void test_ssrf_block_list(void)
{
    ...
    ASSERT_EQ_INT(kelp_ssrf_check("http://8.8.8.8/"), 0);

    /* Add to block list -- should now be blocked */
    kelp_ssrf_block_list_add("8.8.8.8");
    ASSERT_EQ_INT(kelp_ssrf_check("http://8.8.8.8/"), -1);
...
static void test_ssrf_block_overrides_allow(void)
{
    ...
    kelp_ssrf_allow_list_add("evil.example.com");
    kelp_ssrf_block_list_add("evil.example.com");
    ASSERT_EQ_INT(kelp_ssrf_check("http://evil.example.com/"), -1);
...
{
    TEST(ssrf_ipv6_url);
    ASSERT_EQ_INT(kelp_ssrf_check("http://[::1]/admin"), -1);
    ASSERT_EQ_INT(kelp_ssrf_check("http://[fe80::1]/"), -1);
    ASSERT_EQ_INT(kelp_ssrf_check("http://[fc00::1]:8080/api"), -1);
    PASS();
}
...
{
    TEST(ssrf_cgnat);
    ASSERT_TRUE(kelp_ssrf_is_private_ip("100.64.0.1"));
    ASSERT_TRUE(kelp_ssrf_is_private_ip("100.127.255.255"));
    /* Just outside CGNAT range */
    ASSERT_FALSE(kelp_ssrf_is_private_ip("100.128.0.1"));
    PASS();
}
...
static void test_url_encode_basic(void)
{
    ...
    char *enc = kelp_http_url_encode("hello world");
    ASSERT_NOT_NULL(enc);
...
static void test_url_encode_special_chars(void)
{
    ...
    char *enc = kelp_http_url_encode("foo=bar&baz=qux");
    ASSERT_NOT_NULL(enc);
...
static void test_url_encode_unreserved(void)
{
    ...
    char *enc = kelp_http_url_encode("hello-world_v1.0~test");
    ASSERT_NOT_NULL(enc);
...
static void test_url_encode_null(void)
{
    ...
    char *enc = kelp_http_url_encode(NULL);
    ASSERT_NULL(enc);
    PASS();
}
...
static void test_url_encode_unicode(void)
{
    ...
    char *enc = kelp_http_url_encode("caf\xC3\xA9");
    ASSERT_NOT_NULL(enc);
...
static void test_url_encode_slash(void)
{
    ...
    char *enc = kelp_http_url_encode("path/to/file");
    ASSERT_NOT_NULL(enc);
...
static void test_header_add_single(void)
{
    ...
    kelp_http_header_t *list = NULL;
    int rc = kelp_http_header_add(&list, "Content-Type", "application/json");
    ASSERT_EQ_INT(rc, 0);
    ASSERT_NOT_NULL(list);
    ASSERT_EQ_STR(list->name, "Content-Type");
    ASSERT_EQ_STR(list->value, "application/json");
    ASSERT_NULL(list->next);
    kelp_http_header_free(list);
    PASS();
}
...
static void test_header_add_multiple(void)
{
    ...
    kelp_http_header_t *list = NULL;
    kelp_http_header_add(&list, "Accept", "text/html");
    kelp_http_header_add(&list, "Authorization", "Bearer token123");
    kelp_http_header_add(&list, "X-Custom", "value");
...
static void test_header_add_null_args(void)
{
    ...
    kelp_http_header_t *list = NULL;
    ASSERT_EQ_INT(kelp_http_header_add(NULL, "X", "Y"), -1);
    ASSERT_EQ_INT(kelp_http_header_add(&list, NULL, "Y"), -1);
    ASSERT_EQ_INT(kelp_http_header_add(&list, "X", NULL), -1);
    ASSERT_NULL(list);
    PASS();
}
...
static int collect_sse_event(const kelp_sse_event_t *ev, void *ud)
{
...
static void tsp_init(test_sse_parser_t *p, kelp_sse_cb cb, void *ud)
{
    memset(p, 0, sizeof(*p));
    p->cb       = cb;
    p->userdata = ud;
...
static void test_heartbeat_is_alive_null(void)
{
    ...
    ASSERT_FALSE(kelp_heartbeat_is_alive(NULL));
    PASS();
}
