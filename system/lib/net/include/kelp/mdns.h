 ...
 * @param ud    User data passed to kelp_mdns_browse().
 */
typedef void (*kelp_mdns_browse_cb)(const char *name, const char *host,
                                     int port, void *ud);

/**
 * Publish a service on the local network via mDNS.
 *
 * @param name  Human-readable service name (e.g. "kelp gateway").
 * @param type  Service type string (e.g. "_kelp._tcp").
 ...
 *
 * @param type  Service type to browse (e.g. "_kelp._tcp").
