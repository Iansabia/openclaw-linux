...
static bool do_ping(const char *url)
{
    ...
    bool alive = (rc == KELP_OK && resp.status_code >= 200 &&
 ...
 */
static void interruptible_sleep(const kelp_heartbeat_t *hb, int seconds)
{
...
{
    kelp_heartbeat_t *hb = (kelp_heartbeat_t *)arg;

    KELP_INFO("heartbeat: started (url=%s, interval=%ds)",
    ...
    while (atomic_load(&hb->running)) {
        ...
        if (alive && !was_alive) {
            KELP_INFO("heartbeat: endpoint is now UP: %s", hb->url);
        } else if (!alive && was_alive) {
            KELP_WARN("heartbeat: endpoint is now DOWN: %s", hb->url);
        }

        KELP_TRACE("heartbeat: ping %s -> %s", hb->url,
                     alive ? "alive" : "dead");

        interruptible_sleep(hb, hb->interval_sec);
    }

    KELP_INFO("heartbeat: thread exiting for %s", hb->url);
    return NULL;
}
                          ...
                          const char *url, int interval_sec)
{
    if (!hb || !url || interval_sec < 1) {
        KELP_ERROR("heartbeat: invalid arguments");
        return -1;
    }
    ...
    hb->url = strdup(url);
    if (!hb->url) {
        KELP_ERROR("heartbeat: allocation failed");
        return -1;
    }
    ...
    if (rc != 0) {
        KELP_ERROR("heartbeat: pthread_create failed (rc=%d)", rc);
        free(hb->url);
        hb->url = NULL;
