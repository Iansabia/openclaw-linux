                           ...
                           void *ud)
{
    ...
    case AVAHI_ENTRY_GROUP_ESTABLISHED:
        KELP_INFO("mDNS: service '%s' published on %s port %d",
                   s_name, s_type, s_port);
        break;
    case AVAHI_ENTRY_GROUP_COLLISION: {
        /* Rename and retry */
        char *alt = avahi_alternative_service_name(s_name);
        KELP_WARN("mDNS: name collision, renaming '%s' -> '%s'",
                   s_name, alt);
        free(s_name);
        s_name = strdup(alt);
        avahi_free(alt);
    ...
    case AVAHI_ENTRY_GROUP_FAILURE:
        KELP_ERROR("mDNS: entry group failure: %s",
                              ...
                              void *ud)
{
    ...
    case AVAHI_CLIENT_S_RUNNING:
        if (!s_group) {
            ...
            if (!s_group) {
                KELP_ERROR("mDNS: avahi_entry_group_new failed");
                avahi_simple_poll_quit(s_poll);
                return;
            }
        }
        if (avahi_entry_group_is_empty(s_group)) {
            ...
            if (ret < 0) {
                KELP_ERROR("mDNS: failed to add service: %s",
                            avahi_strerror(ret));
                avahi_simple_poll_quit(s_poll);
                return;
            }
    ...
    case AVAHI_CLIENT_FAILURE:
        KELP_ERROR("mDNS: client failure: %s",
                       ...
                       void *ud)
{
    ...
    if (event == AVAHI_RESOLVER_FOUND) {
        ...
        KELP_WARN("mDNS: resolve failed for '%s': %s", name,
                   avahi_strerror(avahi_client_errno(
                      ...
                      void *ud)
{
    ...
    case AVAHI_BROWSER_FAILURE:
        KELP_ERROR("mDNS: browser failure: %s",
                    avahi_strerror(avahi_client_errno(
                        avahi_service_browser_get_client(b))));
        break;
    default:
...
int kelp_mdns_publish(const char *name, const char *type, int port)
{
    if (!name || !type || port <= 0) {
        KELP_ERROR("mDNS: invalid publish arguments");
        return -1;
    }
    ...
    s_port = port;
    if (!s_name || !s_type) {
        KELP_ERROR("mDNS: allocation failed");
        kelp_mdns_unpublish();
        return -1;
    }
    ...
    if (!s_poll) {
        KELP_ERROR("mDNS: avahi_simple_poll_new failed");
        kelp_mdns_unpublish();
        return -1;
    }
    ...
    if (!s_client) {
        KELP_ERROR("mDNS: avahi_client_new failed: %s",
...
int kelp_mdns_browse(const char *type, kelp_mdns_browse_cb cb, void *ud)
{
    if (!type || !cb)
        return -1;
    ...
    AvahiClient *client = avahi_client_new(avahi_simple_poll_get(poll), 0,
                                           NULL, NULL, &error);
    if (!client) {
        KELP_ERROR("mDNS: browse client failed: %s", avahi_strerror(error));
    ...
    AvahiServiceBrowser *browser = avahi_service_browser_new(
        ...
        0, browse_cb, &bud);
    if (!browser) {
        KELP_ERROR("mDNS: browser creation failed");
...
int kelp_mdns_publish(const char *name, const char *type, int port)
{
    ...
    KELP_WARN("mDNS: Avahi not available -- publish is a no-op");
    return -1;
}
...
int kelp_mdns_browse(const char *type, kelp_mdns_browse_cb cb, void *ud)
{
    ...
    KELP_WARN("mDNS: Avahi not available -- browse is a no-op");
    return -1;
}
