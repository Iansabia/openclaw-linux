...
static int ollama_complete(kelp_provider_t *p,
                           const kelp_completion_opts_t *opts,
                           kelp_completion_t *result);

/* ---- Initialization ----------------------------------------------------- */

int kelp_provider_ollama_init(kelp_provider_t *p)
{
    if (!p) return -1;
    ...
    if (opts->tools_json) {
        ...
        if (src_tools && cJSON_IsArray(src_tools)) {
            ...
            cJSON_ArrayForEach(tool_item, src_tools) {
                const char *name = kelp_json_get_string(tool_item, "name");
                const char *desc = kelp_json_get_string(tool_item, "description");
            ...
            KELP_DEBUG("ollama: added %d tools to request",
...
static int parse_response(const char *body, kelp_completion_t *result)
{
    ...
    if (!root) {
        KELP_ERROR("ollama: failed to parse response JSON");
        return -1;
    }

    /* Check for error */
    const char *error_msg = kelp_json_get_string(root, "error");
    if (error_msg) {
        KELP_ERROR("ollama API error: %s", error_msg);
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (message) {
        ...
        if (tool_calls && cJSON_IsArray(tool_calls) &&
            ...
            cJSON_ArrayForEach(tc_item, tool_calls) {
                ...
                const char *tc_name = kelp_json_get_string(fn, "name");
                if (!tc_name) continue;
                ...
                kelp_message_t *tc_msg = kelp_message_new(
                    KELP_ROLE_ASSISTANT, content ? content : "");
                if (tc_msg) {
                    tc_msg->tool_call_id = strdup(tc_id);
                    tc_msg->tool_name    = strdup(tc_name);
                    tc_msg->tool_args    = args_str ? args_str : strdup("{}");
                    kelp_message_append(&result->tool_calls, tc_msg);
                } else {
                    free(args_str);
                }
            }
            KELP_INFO("ollama: parsed %d tool calls", tc_idx);
        }
    }

    /* Ollama reports done_reason (only set if no tool calls) */
    if (!result->stop_reason) {
        bool done = kelp_json_get_bool(root, "done", false);
        if (done) {
    ...
    /* Token counts */
    result->input_tokens  = kelp_json_get_int(root, "prompt_eval_count", 0);
    result->output_tokens = kelp_json_get_int(root, "eval_count", 0);
                           ...
                           kelp_completion_t *result)
{
    ...
    if (!body) {
        KELP_ERROR("ollama: failed to build request JSON");
        return -1;
    }
    ...
    if (rc == 0 && resp.body) {
        ...
        if (resp_body) {
            ...
            if (resp.status_code >= 200 && resp.status_code < 300) {
                ...
                KELP_ERROR("ollama: HTTP %d: %s", resp.status_code, resp_body);
                rc = -1;
            }
            free(resp_body);
        } else {
        ...
        KELP_ERROR("ollama: HTTP request failed (is Ollama running?)");
    }
