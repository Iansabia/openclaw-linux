...
static int google_complete(kelp_provider_t *p,
                           const kelp_completion_opts_t *opts,
                           kelp_completion_t *result);

/* ---- Initialization ----------------------------------------------------- */

int kelp_provider_google_init(kelp_provider_t *p)
{
    if (!p) return -1;
                                 ...
                                 const kelp_completion_opts_t *opts)
{
    ...
    for (kelp_message_t *msg = opts->messages; msg; msg = msg->next) {
        ...
        if (msg->role == KELP_ROLE_ASSISTANT) role = "model";
        ...
        /* Tool call results */
        if (msg->role == KELP_ROLE_TOOL && msg->tool_name) {
            cJSON *part = cJSON_CreateObject();
        ...
        if (msg->role == KELP_ROLE_ASSISTANT && msg->tool_name) {
            cJSON *part = cJSON_CreateObject();
...
static int parse_response(const char *body, kelp_completion_t *result)
{
    ...
    if (!root) {
        KELP_ERROR("google: failed to parse response JSON");
        return -1;
    }
    ...
    if (error_obj) {
        ...
        KELP_ERROR("google API error: %s", emsg ? emsg : "unknown");
        cJSON_Delete(root);
        return -1;
    }
    ...
    if (candidates && cJSON_IsArray(candidates)) {
        ...
        if (cand) {
            /* finish_reason */
            const char *finish = kelp_json_get_string(cand, "finishReason");
            if (finish) {
            ...
            if (content) {
                ...
                if (parts && cJSON_IsArray(parts)) {
                    ...
                    cJSON_ArrayForEach(part, parts) {
                        const char *text = kelp_json_get_string(part, "text");
                        if (text) {
                        ...
                        if (func_call) {
                            const char *name = kelp_json_get_string(func_call, "name");
                            ...
                            if (tc) {
                                ...
                                kelp_str_t id = kelp_str_new();
                                kelp_str_printf(&id, "call_%s", name ? name : "unknown");
                                ...
                                kelp_message_append(&result->tool_calls, tc);
    ...
    if (usage) {
        result->input_tokens  = kelp_json_get_int(usage, "promptTokenCount", 0);
        result->output_tokens = kelp_json_get_int(usage, "candidatesTokenCount", 0);
    }
                           ...
                           kelp_completion_t *result)
{
    ...
    if (!body) {
        KELP_ERROR("google: failed to build request JSON");
        return -1;
    }
    ...
    kelp_str_printf(&url, "%s/%s:generateContent", base, model);
    if (p->api_key) {
        kelp_str_printf(&url, "?key=%s", p->api_key);
    }
    ...
    if (rc == 0 && resp.body) {
        ...
        if (resp_body) {
            ...
            if (resp.status_code >= 200 && resp.status_code < 300) {
                ...
                KELP_ERROR("google: HTTP %d: %s", resp.status_code, resp_body);
                rc = -1;
            }
            free(resp_body);
        } else {
        ...
        KELP_ERROR("google: HTTP request failed");
    }
