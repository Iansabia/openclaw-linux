...
static int make_root_private(void)
{
    if (mount("none", "/", NULL, MS_REC | MS_PRIVATE, NULL) != 0) {
        KELP_ERROR("mount: failed to make root private: %s", strerror(errno));
        return -1;
    }
    return 0;
}
...
static int make_root_readonly(void)
{
    if (mount("none", "/", NULL, MS_REMOUNT | MS_RDONLY | MS_BIND, NULL) != 0) {
        KELP_WARN("mount: failed to remount root read-only: %s", strerror(errno));
...
static int bind_mount(const char *src, const char *dst, bool readonly)
{
    ...
    if (mount(src, dst, NULL, MS_BIND | MS_REC, NULL) != 0) {
        KELP_ERROR("mount: bind mount %s -> %s failed: %s",
    ...
    if (readonly) {
        if (mount(NULL, dst, NULL, MS_REMOUNT | MS_BIND | MS_RDONLY | MS_REC, NULL) != 0) {
            KELP_WARN("mount: failed to make %s read-only: %s", dst, strerror(errno));
        }
    }

    KELP_DEBUG("mount: bind %s -> %s %s", src, dst, readonly ? "(ro)" : "(rw)");
    return 0;
}
...
static int mount_tmpfs(const char *path, size_t size_mb)
{
    ...
    if (mount("tmpfs", path, "tmpfs", MS_NOSUID | MS_NODEV, opts) != 0) {
        KELP_ERROR("mount: tmpfs at %s failed: %s", path, strerror(errno));
        return -1;
    }

    KELP_DEBUG("mount: tmpfs at %s (%zuMB)", path, size_mb);
    return 0;
}
...
static int mount_proc(void)
{
    ...
    if (mount("proc", "/proc", "proc", MS_NOSUID | MS_NODEV | MS_NOEXEC, NULL) != 0) {
        KELP_ERROR("mount: /proc failed: %s", strerror(errno));
        return -1;
    }

    KELP_DEBUG("mount: /proc mounted");
    return 0;
}
 ...
 */
int kelp_mount_setup(const char *workspace,
                      ...
                      int readonly_count)
{
    ...
    (void)readonly_paths;
    (void)readonly_count;
    KELP_WARN("mount: mount namespace setup not available on this platform");
    return -1;
}
