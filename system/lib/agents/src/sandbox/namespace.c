 ...
 */
int kelp_ns_write_id_map(pid_t pid, const char *map_file,
                          ...
                          unsigned int outside_id)
{
    ...
    if (fd < 0) {
        KELP_ERROR("namespace: cannot open %s: %s", path, strerror(errno));
        return -1;
    }
    ...
    if (write(fd, mapping, (size_t)len) != len) {
        KELP_ERROR("namespace: failed to write %s: %s", path, strerror(errno));
        close(fd);
        return -1;
    }
 ...
 */
int kelp_ns_deny_setgroups(pid_t pid)
{
    char path[256];
 ...
 */
int kelp_ns_setup_user_mapping(pid_t child_pid, uid_t host_uid, gid_t host_gid)
{
    /* Must deny setgroups before writing gid_map */
    if (kelp_ns_deny_setgroups(child_pid) != 0) {
        KELP_WARN("namespace: failed to deny setgroups");
    }

    if (kelp_ns_write_id_map(child_pid, "uid_map", 0, (unsigned int)host_uid) != 0) {
        return -1;
    }

    if (kelp_ns_write_id_map(child_pid, "gid_map", 0, (unsigned int)host_gid) != 0) {
        return -1;
    }
...
int kelp_ns_setup_user_mapping(pid_t child_pid, uid_t host_uid, gid_t host_gid)
{
    ...
    (void)host_uid;
    (void)host_gid;
    KELP_WARN("namespace: user namespaces not available on this platform");
    return -1;
}
