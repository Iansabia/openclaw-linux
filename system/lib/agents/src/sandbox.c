...
extern bool kelp_ns_user_ns_available(void);
extern int  kelp_ns_setup_user_mapping(pid_t child_pid, uid_t host_uid, gid_t host_gid);
extern int  kelp_seccomp_apply_filter(void);
extern int  kelp_mount_setup(const char *workspace, const char **readonly_paths, int readonly_count);

/* cgroup interface */
extern kelp_cgroup_t *kelp_cgroup_create(unsigned int sandbox_id);
extern int  kelp_cgroup_set_memory(kelp_cgroup_t *cg, int limit_mb);
extern int  kelp_cgroup_set_cpu(kelp_cgroup_t *cg, int cores);
extern int  kelp_cgroup_set_pids(kelp_cgroup_t *cg, int max_pids);
extern int  kelp_cgroup_add_pid(kelp_cgroup_t *cg, pid_t pid);
extern void kelp_cgroup_destroy(kelp_cgroup_t *cg);
...
{
#ifdef __linux__
    return kelp_ns_user_ns_available();
#else
    return false;
#endif
}
...
kelp_sandbox_t *kelp_sandbox_new(const kelp_sandbox_opts_t *opts)
{
    ...
    kelp_sandbox_t *sb = (kelp_sandbox_t *)calloc(1, sizeof(*sb));
    if (!sb) return NULL;
    ...
    /* Create cgroup */
    sb->cgroup = kelp_cgroup_create(sb->id);
    if (sb->cgroup) {
        kelp_cgroup_set_memory(sb->cgroup, sb->opts.memory_limit_mb);
        kelp_cgroup_set_cpu(sb->cgroup, sb->opts.cpu_cores);
        kelp_cgroup_set_pids(sb->cgroup, sb->opts.max_pids);
    }
#endif

    KELP_DEBUG("sandbox: created sandbox %u", sb->id);
    return sb;
}
...
typedef struct {
    const char  *cmd;
    char *const *argv;
    const kelp_sandbox_opts_t *opts;
...
static int child_fn(void *arg)
{
    ...
    kelp_mount_setup(ca->opts->workspace,
                       ...
                       char *const argv[], char **output, size_t *output_len)
{
    ...
    int pipefd[2];
    if (pipe(pipefd) != 0) {
        KELP_ERROR("sandbox: pipe() failed: %s", strerror(errno));
        return -1;
    }
    ...
    if (child < 0) {
        KELP_ERROR("sandbox: clone() failed: %s", strerror(errno));
        free(stack);
        close(pipefd[0]); close(pipefd[1]);
        close(outfd[0]); close(outfd[1]);
        return -1;
    ...
    kelp_ns_setup_user_mapping(child,
                                 sb->opts.uid_map_host,
                                 sb->opts.gid_map_host);

    /* Add child to cgroup */
    if (sb->cgroup) {
        kelp_cgroup_add_pid(sb->cgroup, child);
    }
    ...
    if (timeout_ms > 0) {
        ...
        if (elapsed >= timeout_ms) {
            KELP_WARN("sandbox: command timed out after %d seconds", sb->opts.timeout_sec);
            kill(child, SIGKILL);
                       ...
                       char *const argv[], char **output, size_t *output_len)
{
    ...
    KELP_ERROR("sandbox: execution not available on this platform");
