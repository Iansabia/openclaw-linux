 ...
 *   - Path traversal detection (kelp_fs_is_safe_path)
 *   - Safe path resolution (kelp_fs_resolve_safe)
 *   - Filesystem permission checks (kelp_fs_check_perm, kelp_fs_check_ownership)
 *   - Policy rule matching (kelp_policy_check)
 *   - Path scanner patterns (kelp_path_scanner_check)
 *   - Audit event formatting (kelp_audit_log)
 *
...
static void test_path_traversal(void)
{
    ...
    TEST_BEGIN("simple safe path") {
        TEST_ASSERT(kelp_fs_is_safe_path("/home/user/documents/file.txt"));
    } TEST_END();

    TEST_BEGIN("relative safe path") {
        TEST_ASSERT(kelp_fs_is_safe_path("foo/bar/baz.c"));
    } TEST_END();

    TEST_BEGIN("single dot in path is safe") {
        TEST_ASSERT(kelp_fs_is_safe_path("./foo/bar"));
    } TEST_END();

    TEST_BEGIN("simple traversal attack") {
        TEST_ASSERT(!kelp_fs_is_safe_path("../../../etc/passwd"));
    } TEST_END();

    TEST_BEGIN("traversal hidden in middle") {
        /* foo/../../ goes above foo's parent */
        TEST_ASSERT(!kelp_fs_is_safe_path("foo/../../secret"));
    } TEST_END();

    TEST_BEGIN("deeply nested traversal") {
        TEST_ASSERT(!kelp_fs_is_safe_path("a/b/c/../../../../etc/shadow"));
    } TEST_END();

    TEST_BEGIN("null path is unsafe") {
        TEST_ASSERT(!kelp_fs_is_safe_path(NULL));
    } TEST_END();

    TEST_BEGIN("empty path is unsafe") {
        TEST_ASSERT(!kelp_fs_is_safe_path(""));
    } TEST_END();
...
static void test_safe_resolution(void)
{
    ...
    TEST_BEGIN("resolve simple relative path") {
        char out[PATH_MAX];
        int rc = kelp_fs_resolve_safe("/tmp", "foo.txt", out, sizeof(out));
    ...
    TEST_BEGIN("reject absolute relative path") {
        char out[PATH_MAX];
        int rc = kelp_fs_resolve_safe("/tmp", "/etc/passwd", out, sizeof(out));
        TEST_ASSERT(rc == -1);
    } TEST_END();
    ...
    TEST_BEGIN("reject null base") {
        char out[PATH_MAX];
        int rc = kelp_fs_resolve_safe(NULL, "foo.txt", out, sizeof(out));
        TEST_ASSERT(rc == -1);
    } TEST_END();

    TEST_BEGIN("reject relative base") {
        char out[PATH_MAX];
        int rc = kelp_fs_resolve_safe("relative/base", "foo.txt",
                                       out, sizeof(out));
        TEST_ASSERT(rc == -1);
    } TEST_END();
}
...
static void test_fs_permissions(void)
{
    ...
    TEST_BEGIN("check read permission on /tmp") {
        int rc = kelp_fs_check_perm("/tmp", KELP_PERM_READ);
        TEST_ASSERT(rc == 0);
    } TEST_END();

    TEST_BEGIN("check nonexistent path") {
        int rc = kelp_fs_check_perm("/nonexistent_path_xyzzy_12345",
                                     KELP_PERM_READ);
        TEST_ASSERT(rc == -1);
    } TEST_END();
    ...
    TEST_BEGIN("check ownership of /tmp") {
        ...
        bool owned = kelp_fs_check_ownership("/tmp", getuid());
    ...
    TEST_BEGIN("check ownership of null path") {
        bool owned = kelp_fs_check_ownership(NULL, 0);
        TEST_ASSERT(!owned);
    } TEST_END();

    TEST_BEGIN("check null path perm") {
        int rc = kelp_fs_check_perm(NULL, KELP_PERM_READ);
        TEST_ASSERT(rc == -1);
    } TEST_END();
}
...
static void test_policy(void)
{
    ...
    TEST_BEGIN("empty policy allows everything") {
        kelp_policy_t *p = kelp_policy_new();
        TEST_ASSERT(p != NULL);

        kelp_policy_action_t a = kelp_policy_check(p, "bash", "ls -la");
        TEST_ASSERT(a == KELP_POLICY_ALLOW);
    ...
    TEST_BEGIN("explicit deny rule") {
        ...
        kelp_policy_rule_t rule = {
            .tool_name = "bash",
            .pattern   = "rm -rf /*",
            .action    = KELP_POLICY_DENY,
            .reason    = "dangerous",
        };
        int rc = kelp_policy_add_rule(p, &rule);
        TEST_ASSERT(rc == 0);

        kelp_policy_action_t a = kelp_policy_check(p, "bash", "rm -rf /home");
        TEST_ASSERT(a == KELP_POLICY_DENY);

        /* Non-matching command should be allowed. */
        a = kelp_policy_check(p, "bash", "echo hello");
        TEST_ASSERT(a == KELP_POLICY_ALLOW);
    ...
    TEST_BEGIN("wildcard tool name") {
        ...
        kelp_policy_add_rule(p, &rule);

        TEST_ASSERT(kelp_policy_check(p, "file_read", ".env") ==
                    KELP_POLICY_DENY);
        TEST_ASSERT(kelp_policy_check(p, "file_write", "prod.env") ==
                    KELP_POLICY_DENY);
        TEST_ASSERT(kelp_policy_check(p, "bash", "cat config.yaml") ==
                    KELP_POLICY_ALLOW);
    ...
    TEST_BEGIN("first match wins") {
        ...
        kelp_policy_rule_t r1 = {
            .tool_name = "bash",
            .pattern   = "echo *",
            .action    = KELP_POLICY_ALLOW,
            .reason    = "echo is safe",
        };
        kelp_policy_add_rule(p, &r1);

        /* Second rule: audit all bash. */
        kelp_policy_rule_t r2 = {
            .tool_name = "bash",
            .pattern   = "*",
            .action    = KELP_POLICY_AUDIT,
            .reason    = "audit all",
        };
        kelp_policy_add_rule(p, &r2);

        /* "echo hello" should match rule 1 (allow), not rule 2 (audit). */
        TEST_ASSERT(kelp_policy_check(p, "bash", "echo hello") ==
                    KELP_POLICY_ALLOW);

        /* "ls" should match rule 2 (audit). */
        TEST_ASSERT(kelp_policy_check(p, "bash", "ls") ==
                    KELP_POLICY_AUDIT);
    ...
    TEST_BEGIN("default rules include deny for rm -rf /") {
        ...
        kelp_policy_add_default_rules(p);

        TEST_ASSERT(kelp_policy_check(p, "bash", "rm -rf /home") ==
                    KELP_POLICY_DENY);
    ...
    TEST_BEGIN("default rules deny writing to /etc") {
        ...
        TEST_ASSERT(kelp_policy_check(p, "file_write", "/etc/passwd") ==
                    KELP_POLICY_DENY);
    ...
    TEST_BEGIN("default rules audit bash commands") {
        ...
        TEST_ASSERT(kelp_policy_check(p, "bash", "ls -la /home") ==
                    KELP_POLICY_AUDIT);
    ...
    TEST_BEGIN("ask action") {
        ...
        kelp_policy_rule_t rule = {
            .tool_name = "bash",
            .pattern   = "sudo *",
            .action    = KELP_POLICY_ASK,
            .reason    = "privilege escalation requires confirmation",
        };
        kelp_policy_add_rule(p, &rule);

        TEST_ASSERT(kelp_policy_check(p, "bash", "sudo apt install foo") ==
                    KELP_POLICY_ASK);
    ...
    TEST_BEGIN("null policy check") {
        TEST_ASSERT(kelp_policy_check(NULL, "bash", "ls") ==
                    KELP_POLICY_ALLOW);
    } TEST_END();
}
...
static void test_path_scanner(void)
{
    ...
    TEST_BEGIN("empty scanner allows everything") {
        kelp_path_scanner_t *s = kelp_path_scanner_new();
        TEST_ASSERT(s != NULL);

        TEST_ASSERT(kelp_path_scanner_check(s, "/home/user/file.txt"));
        TEST_ASSERT(kelp_path_scanner_check(s, "/etc/passwd"));

        kelp_path_scanner_free(s);
    } TEST_END();
    ...
    TEST_BEGIN("deny .env files") {
        ...
        TEST_ASSERT(!kelp_path_scanner_check(s, "/app/.env"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/app/production.env"));
        TEST_ASSERT(kelp_path_scanner_check(s, "/app/config.yaml"));
    ...
    TEST_BEGIN("deny .ssh paths") {
        ...
        TEST_ASSERT(!kelp_path_scanner_check(s, ".ssh/id_rsa"));
        TEST_ASSERT(!kelp_path_scanner_check(s, ".ssh/config"));
        TEST_ASSERT(kelp_path_scanner_check(s, "/home/user/doc.txt"));
    ...
    TEST_BEGIN("deny key files") {
        ...
        TEST_ASSERT(!kelp_path_scanner_check(s, "/certs/server.pem"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/certs/server.key"));
        TEST_ASSERT(kelp_path_scanner_check(s, "/certs/server.crt"));
    ...
    TEST_BEGIN("deny /proc and /sys") {
        ...
        TEST_ASSERT(!kelp_path_scanner_check(s, "/proc/self/maps"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/sys/class/net"));
        TEST_ASSERT(kelp_path_scanner_check(s, "/var/log/syslog"));
    ...
    TEST_BEGIN("first match wins (allow overrides later deny)") {
        ...
        kelp_path_scanner_add_pattern(s, "*.env.example", false);
        /* Then deny all .env. */
        kelp_path_scanner_add_pattern(s, "*.env", true);

        /* .env.example should be allowed (first rule matches). */
        TEST_ASSERT(kelp_path_scanner_check(s, "/app/.env.example"));
    ...
    TEST_BEGIN("default patterns deny sensitive files") {
        ...
        kelp_path_scanner_add_defaults(s);

        /* These should all be denied. */
        TEST_ASSERT(!kelp_path_scanner_check(s, "/app/.env"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/app/prod.env"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/home/user/.ssh/id_rsa"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/certs/ca.pem"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/certs/server.key"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/proc/self/environ"));
        TEST_ASSERT(!kelp_path_scanner_check(s, "/sys/class/net/eth0"));

        /* These should be allowed. */
        TEST_ASSERT(kelp_path_scanner_check(s, "/home/user/code/main.c"));
        TEST_ASSERT(kelp_path_scanner_check(s, "/var/log/syslog"));
        TEST_ASSERT(kelp_path_scanner_check(s, "/home/user/README.md"));
    ...
    TEST_BEGIN("null path is denied") {
        ...
        TEST_ASSERT(!kelp_path_scanner_check(s, NULL));

        kelp_path_scanner_free(s);
    } TEST_END();

    TEST_BEGIN("null scanner is denied") {
        TEST_ASSERT(!kelp_path_scanner_check(NULL, "/some/path"));
    } TEST_END();
}
...
static kelp_audit_event_t last_sink_event;
static int sink_call_count = 0;

static void test_sink(const kelp_audit_event_t *event, void *userdata)
{
    (void)userdata;
    if (event) {
...
static void test_audit(void)
{
    ...
    TEST_BEGIN("init and shutdown") {
        ...
        char path[] = "/tmp/kelp_test_audit_XXXXXX";
        int fd = mkstemp(path);
        TEST_ASSERT(fd >= 0);
        close(fd);

        int rc = kelp_audit_init(path);
        TEST_ASSERT(rc == 0);
    ...
    TEST_BEGIN("log event writes JSON line") {
        ...
        kelp_audit_event_t ev = {
            .level    = KELP_AUDIT_INFO,
    ...
    TEST_BEGIN("min level filtering") {
        ...
        kelp_audit_set_min_level(KELP_AUDIT_ALERT);

        /* This event should be filtered out (INFO < ALERT). */
        kelp_audit_event_t ev_info = {
            .level    = KELP_AUDIT_INFO,
            .timestamp = 1700000000,
            .category = "test",
            .action   = "noop",
        ...
        kelp_audit_log(&ev_info);

        /* This event should pass (ALERT >= ALERT). */
        kelp_audit_event_t ev_alert = {
            .level    = KELP_AUDIT_ALERT,
            .timestamp = 1700000000,
            .category = "test",
            .action   = "alert",
            .subject  = "test",
    ...
    TEST_BEGIN("sink callback receives events") {
        ...
        rc = kelp_audit_add_sink(test_sink, NULL);
        TEST_ASSERT(rc == 0);

        kelp_audit_event_t ev = {
            .level    = KELP_AUDIT_WARN,
            .timestamp = 1700000000,
            .category = "tool",
            .action   = "exec",
            .subject  = "user-42",
        ...
        TEST_ASSERT(last_sink_event.level == KELP_AUDIT_WARN);
...
static void test_timing_safe_cmp(void)
{
    ...
    TEST_BEGIN("equal strings return true") {
        ...
        TEST_ASSERT(kelp_timing_safe_cmp(a, b, 16));
    } TEST_END();
    ...
    TEST_BEGIN("equal binary data returns true") {
        ...
        TEST_ASSERT(kelp_timing_safe_cmp(a, b, sizeof(a)));
    } TEST_END();
    ...
    TEST_BEGIN("single byte difference returns false") {
        ...
        TEST_ASSERT(!kelp_timing_safe_cmp(a, b, 8));
    } TEST_END();
    ...
    TEST_BEGIN("zero-length comparison returns true") {
        const char *a = "hello";
        const char *b = "world";
        TEST_ASSERT(kelp_timing_safe_cmp(a, b, 0));
    } TEST_END();

    TEST_BEGIN("both NULL with zero length returns true") {
        TEST_ASSERT(kelp_timing_safe_cmp(NULL, NULL, 0));
    } TEST_END();

    TEST_BEGIN("NULL first pointer returns false") {
        const char *b = "hello";
        TEST_ASSERT(!kelp_timing_safe_cmp(NULL, b, 5));
    } TEST_END();

    TEST_BEGIN("NULL second pointer returns false") {
        const char *a = "hello";
        TEST_ASSERT(!kelp_timing_safe_cmp(a, NULL, 5));
    } TEST_END();

    TEST_BEGIN("NULL pointer with zero length returns false") {
        const char *a = "hello";
        TEST_ASSERT(!kelp_timing_safe_cmp(NULL, a, 0));
        TEST_ASSERT(!kelp_timing_safe_cmp(a, NULL, 0));
    } TEST_END();

    TEST_BEGIN("both NULL with non-zero length returns false") {
        TEST_ASSERT(!kelp_timing_safe_cmp(NULL, NULL, 10));
    } TEST_END();

    TEST_BEGIN("single byte equal") {
        unsigned char a = 0x42;
        unsigned char b = 0x42;
        TEST_ASSERT(kelp_timing_safe_cmp(&a, &b, 1));
    } TEST_END();
...
int main(void)
{
    fprintf(stderr, "libkelp-security test suite\n");
    fprintf(stderr, "============================\n");
