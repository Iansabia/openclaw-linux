...
static struct {
    ...
    .fp          = NULL,
    .initialised = 0,
    .min_level   = KELP_AUDIT_INFO,
    .sink_count  = 0,
};

/* ---- helpers ------------------------------------------------------------ */

static const char *level_string(kelp_audit_level_t level)
{
    switch (level) {
    case KELP_AUDIT_INFO:      return "info";
    case KELP_AUDIT_WARN:      return "warn";
    case KELP_AUDIT_ALERT:     return "alert";
    case KELP_AUDIT_VIOLATION: return "violation";
...
int kelp_audit_init(const char *log_path)
{
    ...
    if (!g_audit.fp) {
        ...
        KELP_ERROR("audit: cannot open log file: %s", log_path);
        return -1;
    }

    g_audit.initialised = 1;
    g_audit.min_level   = KELP_AUDIT_INFO;
    g_audit.sink_count  = 0;

    pthread_mutex_unlock(&g_audit.mutex);

    KELP_INFO("audit: initialised, log_path=%s", log_path);
    return 0;
}
...
int kelp_audit_add_sink(kelp_audit_sink_t sink, void *userdata)
{
    ...
    if (g_audit.sink_count >= AUDIT_MAX_SINKS) {
        pthread_mutex_unlock(&g_audit.mutex);
        KELP_WARN("audit: maximum number of sinks reached (%d)",
                   AUDIT_MAX_SINKS);
        return -1;
    }
