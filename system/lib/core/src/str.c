...
static int str_grow(kelp_str_t *s, size_t needed)
{
...
kelp_str_t kelp_str_from(const char *src)
{
    kelp_str_t s = {0};
    if (!src)
        return kelp_str_new();
...
{
    if (!cstr) return 0;
    return kelp_str_append(s, cstr, strlen(cstr));
}
