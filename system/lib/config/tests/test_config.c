...
static const char *sample_yaml =
    ...
    "  host: 0.0.0.0\n"
    "  port: 9090\n"
    "  socket_path: /tmp/kelp-test.sock\n"
    "  tls_enabled: false\n"
    "\n"
    "model:\n"
...
write_temp_yaml(void)
{
    snprintf(tmp_yaml_path, sizeof(tmp_yaml_path),
             "/tmp/kelp_test_%d.yaml", (int)getpid());
...
test_load_yaml(void)
{
    ...
    ASSERT_EQ_INT(cfg.logging.level, KELP_LOG_DEBUG);
    ASSERT_EQ_STR(cfg.logging.file, "/var/log/kelp/test.log");

    kelp_config_free(&cfg);
}
...
{
    kelp_config_t cfg;
    int rc = kelp_config_load("/no/such/file.yaml", &cfg);
    ASSERT_EQ_INT(rc, -1);
}
...
{
    /* Make sure KELP_CONFIG_DIR doesn't point at a real file for this test */
    unsetenv("KELP_CONFIG_DIR");

    kelp_config_t cfg;
    int rc = kelp_config_load_default(&cfg);
    ASSERT_EQ_INT(rc, 0);
    ...
    ASSERT_TRUE(cfg.security.sandbox_enabled);
    ASSERT_EQ_INT(cfg.logging.level, KELP_LOG_INFO);
...
test_merge_env(void)
{
    ...
    /* Override via env */
    setenv("KELP_HOST", "10.0.0.1", 1);
    setenv("KELP_PORT", "3000", 1);
    setenv("KELP_PROVIDER", "openai", 1);
    setenv("KELP_LOG_LEVEL", "warn", 1);
    setenv("KELP_SANDBOX", "no", 1);

    kelp_config_merge_env(&cfg);
    ...
    ASSERT_EQ_INT(cfg.logging.level, KELP_LOG_WARNING);
    ASSERT_TRUE(!cfg.security.sandbox_enabled);
    ...
    unsetenv("KELP_HOST");
    unsetenv("KELP_PORT");
    unsetenv("KELP_PROVIDER");
    unsetenv("KELP_LOG_LEVEL");
    unsetenv("KELP_SANDBOX");
}
...
test_get_string(void)
{
    ...
    ASSERT_EQ_STR(kelp_config_get_string(&cfg, "gateway.host"), "0.0.0.0");
    ASSERT_EQ_STR(kelp_config_get_string(&cfg, "model.default_provider"), "anthropic");
    ASSERT_TRUE(kelp_config_get_string(&cfg, "nonexistent.key") == NULL);
...
test_get_int(void)
{
    ...
    ASSERT_EQ_INT(kelp_config_get_int(&cfg, "gateway.port", -1), 9090);
    ASSERT_EQ_INT(kelp_config_get_int(&cfg, "model.max_tokens", -1), 8192);
    ASSERT_EQ_INT(kelp_config_get_int(&cfg, "nonexistent", 42), 42);
...
test_get_bool(void)
{
    ...
    ASSERT_TRUE(kelp_config_get_bool(&cfg, "security.sandbox_enabled", false));
    ASSERT_TRUE(!kelp_config_get_bool(&cfg, "gateway.tls_enabled", true));
    ASSERT_TRUE(kelp_config_get_bool(&cfg, "nonexistent", true));
...
test_validate_good(void)
{
    ...
    int rc = kelp_config_load(tmp_yaml_path, &cfg);
    ASSERT_EQ_INT(rc, 0);
    ASSERT_EQ_INT(kelp_config_validate(&cfg), 0);
    kelp_config_free(&cfg);
}
...
test_validate_bad_port(void)
{
    ...
    ASSERT_EQ_INT(kelp_config_validate(&cfg), -1);
...
test_schema_validate_good(void)
{
    ...
    const kelp_schema_t *schema = kelp_schema_config();
    char err[256] = {0};
    int rc = kelp_schema_validate(schema, root, err, sizeof(err));
    ASSERT_EQ_INT(rc, 0);
...
test_paths_config_dir(void)
{
    ...
    char *dir = kelp_paths_config_dir();
    ASSERT_NOT_NULL(dir);
    /* Should end with ".config/kelp" */
    ASSERT_TRUE(strlen(dir) > 13);
    const char *tail = dir + strlen(dir) - 13;
    ASSERT_EQ_STR(tail, ".config/kelp");
...
{
    setenv("KELP_CONFIG_DIR", "/opt/kelp/conf", 1);

    char *dir = kelp_paths_config_dir();
    ASSERT_NOT_NULL(dir);
    ASSERT_EQ_STR(dir, "/opt/kelp/conf");
...
test_paths_expand_tilde(void)
{
    ...
    char *result = kelp_paths_expand("~/foo/bar");
...
{
    setenv("KELP_TEST_VAR", "hello", 1);

    char *result = kelp_paths_expand("/prefix/${KELP_TEST_VAR}/suffix");
    ASSERT_NOT_NULL(result);
    ASSERT_EQ_STR(result, "/prefix/hello/suffix");

    free(result);
    unsetenv("KELP_TEST_VAR");
}

static void
test_paths_expand_default(void)
{
    unsetenv("KELP_UNSET_VAR");

    char *result = kelp_paths_expand("/path/${KELP_UNSET_VAR:-fallback}/end");
    ASSERT_NOT_NULL(result);
...
{
    char *sock = kelp_paths_socket();
    ASSERT_NOT_NULL(sock);
    ASSERT_TRUE(strstr(sock, "kelp.sock") != NULL);
    free(sock);
}
...
write_temp_json(void)
{
    snprintf(tmp_json_path, sizeof(tmp_json_path),
             "/tmp/kelp_test_%d.json", (int)getpid());
...
test_load_json(void)
{
    ...
    kelp_config_t cfg;
    int rc = kelp_config_load(tmp_json_path, &cfg);
    ASSERT_EQ_INT(rc, 0);
    ...
    ASSERT_EQ_INT(cfg.logging.level, KELP_LOG_ERR);
...
main(void)
{
    printf("libkelp-config test suite\n");
    printf("==========================\n\n");
