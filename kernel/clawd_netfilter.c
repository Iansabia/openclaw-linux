/*
 * clawd_netfilter.c — Netfilter hook for network-aware AI
 *
 * Hooks into the LOCAL_OUT chain to log/inspect outbound connections.
 * Events are made available to userspace via /proc/clawd/netfilter
 ...
 */
static unsigned int clawd_nf_hook(void *priv,
                                   struct sk_buff *skb,
                                   const struct nf_hook_state *state)
{
    ...
    if (iph->protocol == IPPROTO_TCP) {
        ...
        if (tcph) {
            ...
            if (tcph->syn && !tcph->ack && clawd_get_log_level() >= 2) {
                pr_info("clawd: TCP SYN %pI4:%d -> %pI4:%d\n",
...
int clawd_netfilter_init(void)
{
    ...
    nf_hook_ops.hook     = clawd_nf_hook;
    ...
    if (ret < 0) {
        pr_err("clawd: failed to register netfilter hook: %d\n", ret);
        return ret;
    }

    pr_info("clawd: netfilter hook registered (LOCAL_OUT)\n");
    return 0;
}
...
void clawd_netfilter_exit(void)
{
    ...
    pr_info("clawd: netfilter hook unregistered\n");
}
